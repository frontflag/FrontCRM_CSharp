...
3. **历史与入口**  
   顶栏铃铛打开右侧消息抽屉（对齐 AI 助手抽屉）：Tab「系统消息」+「系统公告」。系统消息已独立实现，见 [系统通知-设计与实现](./系统通知-设计与实现.md)。顶栏角标 = 未读系统通知 + 未读系统公告。
...
| 系统消息业务内容 | 已拆为独立功能，见 [系统通知-设计与实现](./系统通知-设计与实现.md) |
...
| Tab | **系统消息**（点对点通知，见 [系统通知](./系统通知-设计与实现.md)）\| **系统公告** |
| 顶栏角标 | = 未读系统通知 + 未读系统公告；`>20` 显示 `20+` |
| 摇动 | 仅未读**重要**系统通知；仅有未读公告不摇 |
...
| 2026-08-24 | 铃铛「系统消息」不再空态；角标改为通知+公告未读之和；摇动规则见系统通知文档 |

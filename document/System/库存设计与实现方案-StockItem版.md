...
**约定**：入库过账时以当时采购行型号 / 品牌快照写入 `stockitem` 与 `stock` 分桶键。之后在采购单上改型号 / 品牌**不会随保存自动回写**；须在采购订单详情显式「刷新物料型号 / 刷新品牌」，由 `StockItemPurchaseIdentityRebucketService` 改层上的键、换堆或新建汇总行，重算桶数量与备货可用量（不改 `StockItemCode`）；换堆后已无明细且数量为 0 的旧汇总行软删。见 [采购订单分面刷新](./采购/采购订单分面刷新-设计与实现.md) §5.3。其它主数据（客户、业务员等）仍不自动回写。

...
| 数据 | `docBind`。**销售装箱 Invoice：** 单价/金额取装箱明细销售价 `packing_item_extend.Price`（缺扩展则回退销售订单行价），**币别按行**打印（`PriceCurrency`，如 `6.75 RMB`）。**报关装箱 Invoice**（仅装箱单入口）：按报关明细拆行，单价用已试算 `cost_usd`，币别 USD；未试算禁打并提示「请先试算费用」。**不要**用出库头 `TotalAmount` 摊数量。无行价时才退回按出库头金额分摊。报关装箱 Invoice 的 Bill To / Ship To 与 Packing 同一套报关公司覆盖。Bank Details 不变。 |

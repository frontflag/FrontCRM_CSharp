...
采购订单明细列表右侧「操作」页签最上「采购订单」卡片：主状态为 **审核通过(10)**、**待确认(20)** 时，在状态后追加斜体浅灰「（等待供应商确认）」；**已确认(30)** 及之后只显示主状态文案。见 [采购订单明细列表-操作面板 §3.0](采购订单明细列表-操作面板-设计与实现.md)。
...
| `10 审核通过` | `20 待确认`、`-2 取消` |
| `20 待确认` | `30 已确认`、`-2 取消` |
| `30 已确认` | `50 进行中`、`20 待确认`（取消确认）、`-2 取消` |
| `50 进行中` | `100 采购完成` |
...
当人工将主单更新为 `1 / 2 / 10 / 20 / 30 / -1 / -2` 时，**全部明细** `purchaseorderitem.status` 与主单同步（`ShouldSyncOrderAndItemStatus`）。

**业务意义**：新建至已确认阶段，主单与明细保持同一审批节点；进入执行阶段（50、100）后，明细按实际业务里程碑独立维护。

### 2.2 取消确认 vs 取消订单

完整业务规则、下游命中条件与代码索引见 **[采购订单取消确认-设计与实现](./采购/采购订单取消确认-设计与实现.md)**。

| 操作 | 入口 | 流转 | 说明 |
|------|------|------|------|
| **取消确认** | 列表操作列；详情顶栏（仅已确认） | **`30 → 20`** | 撤回供应商确认，**不是**整单作废。退回后只能再点「供应商确认」回到 30。`50/100` 不显示此按钮。存在有效 **付款单**（未删除、非取消/非审核失败，含在途请款）、**到货通知**、已过账 **采购入库**、**进项发票** 时禁止。货代单号不阻断。实现：`PurchaseOrderRevertVendorConfirmGuard`。 |
| **取消订单** | 详情顶栏 | 主状态 **&lt; 10**（新建/待审核/审核失败）→ **`-2`** | 整单作废。待确认(20) 及之后 **无** 此按钮。 |

后端仍允许 `30 → -2`（应急/改库，**不走** Guard）；界面「取消确认」只发 `20`，不再发送 `-2`。目标为 `20` 时明细 `status` 随主单同步（§2.1）。
...
| 详情页「刷新状态」 | `PurchaseOrderService.RefreshDownstreamAsync(Status)`：逐行 `RecalculateAsync(StatusOnly)` + 同步明细 `status` + 同步主状态；不改到货单价/品牌/计划量 |
| Debug 批量修复 | `POST /api/v1/debug/refresh-purchase-order-main-status`：全部采购订单逐单 `RefreshDownstreamAsync(Status)`；跳过 `-1/-2`；`/debug/data` 页「刷新采购订单状态」 |
...
有效入库单：`stockin.status = 已入库(2)`，且 `StockInType` 为 **采购入库 `10`**（`StockInTypeCode.IsPurchaseReceipt`；代码仍兼容历史 `1`，**库中已刷净**）。报关入库 `20` 为后续入境内，**不计入**采购行收货；移库 `3`、退货、报废及未识别类型同样不计入。经 `stockinitemextend.purchase_order_item_id` 关联本行。详情底部「入库」页签仍可列出挂到本行的其它类型入库单（溯源），**不驱动**本状态与 `qty_receive_total`。刷新详情页头「刷新」会重算本字段。销售明细入库进度本期不改。

### 4.3 付款状态（`payment_progress_status`）

相对行应付 `payment_amount`（= `qty × cost`）与已核销 `payment_amount_finish`。明细表 `finance_payment_status`、主单 `finance_status` **同一公式**（`PurchaseLineFinancePaymentStatus`）：

| 状态值 | 条件 |
|--------|------|
| 0 未付款 | 已核销合计 `== 0` |
| 1 部分付款 | `0 <` 已核销合计 `<` 行采购总额 |
| 2 付款完成 | 已核销合计 `>=` 行采购总额 |

有效付款单排除取消、审核失败、软删除。已核销不含在途未付的请款。申请付款门槛：扩展 `payment_progress_status < 2`（即已付 < 行总额；与回写后的 `finance_payment_status` 同口径），另由可申请金额（总额 − 已申请）拦住请款已满。
...
另：`stock_in_status`、`finance_payment_status`、`stock_out_status` 为 **0/1/2 汇总态**（未/部分/完成）。`finance_payment_status` 与扩展 `payment_progress_status` 同口径（已核销 vs 行总额）；列表「付款状态」列以扩展进度为准。

### 5.1 刷新时自动对齐（`ComputeItemStatusAfterRefresh`）

在详情「刷新状态」（及刷新数量/单价之后的重算）中，扩展重算后按事实 **上调或下调** 明细 `status`（取消/审核失败行不改）：
...
| 明细 status 刷新 | `PurchaseOrderService.ComputeItemStatusAfterRefresh`、`RefreshDownstreamAsync(Status)` |
| 扩展字段模型注释 | `CRM.Core/Models/Purchase/PurchaseOrderItemExtend.cs` |
| 前端主状态文案 | `CRM.Web/src/constants/purchaseOrderStatus.ts` |
| 明细列表操作页签主单状态展示 | [采购订单明细列表-操作面板 §3.0](采购订单明细列表-操作面板-设计与实现.md)；`purchaseOrderMainStatusAwaitingVendorConfirm` |
| 取消确认 | [采购订单取消确认-设计与实现](./采购/采购订单取消确认-设计与实现.md)；`PurchaseOrderRevertVendorConfirmGuard`；QA [采购订单取消确认-测试对照说明](../QA/采购/采购订单取消确认-测试对照说明.md) |
| 列表取消行样式 | 主单取消(-2) 时采购订单/明细列表整行浅灰（明细只看 `orderStatus`）；审核失败不变灰。见 [业务列表取消行样式-设计与实现](./系统/业务列表取消行样式-设计与实现.md) |
...
*文档维护：2026-09-01 · 取消确认口径与实现见独立设计文档；明细列表操作页签审核通过/待确认追加「（等待供应商确认）」*

...
有效入库单：`stockin.status = 已入库(2)`，且 `StockInType` 为 **采购入库 `10` 或历史 `1`**（`StockInTypeCode.IsPurchaseReceipt`）。报关入库 `20` 为后续入境内，**不计入**采购行收货；移库 `3`、退货、报废及未识别类型同样不计入。经 `stockinitemextend.purchase_order_item_id` 关联本行。详情底部「入库」页签仍可列出挂到本行的其它类型入库单（溯源），**不驱动**本状态与 `qty_receive_total`。刷新详情页头「刷新」会重算本字段。销售明细入库进度本期不改。

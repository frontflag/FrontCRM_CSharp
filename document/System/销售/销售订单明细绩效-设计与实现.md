...
| **预计销售** | `SalesProfitExpected` | `收入 USD ÷ PoCostUsdConfirmed`；已确认成本为 0 → `—` | 同上 |
...
中间层 **不** 单独展示「采购利润（全 PO）」；存在已下单未确认成本时，用 info 说明提示。
...
| 场景 | 行为 |
|------|------|
| 销售订单详情「刷新扩展」 | `POST .../refresh-item-extends` → `RecalculateAsync` 写回 extend |
| Debug 批量 | `POST /api/v1/debug/refresh-sellorderitemextend-outbound-profit` 逐行重算 |

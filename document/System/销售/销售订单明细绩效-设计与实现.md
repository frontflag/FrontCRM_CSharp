...
| 场景 | 行为 |
|------|------|
| 销售订单详情「刷新扩展」 | `POST .../refresh-item-extends` → `RecalculateAsync` 写回 extend（含 `SalesProfitExpected`） |
...
**列表 vs 绩效：** 明细列表「预计销售利润」读落库 `sellorderitemextend.SalesProfitExpected`；绩效面板 `line-profit` **实时**按瀑布重算。公式改版后历史行不会自动变，需详情「刷新」或 Debug 批量写回。备货出库若扩展行误挂备货原销售行，重算加载批次成本与出库数量归属一致（头表挂客单行 + 备货/样品/空绑定）。

...
新建需求页默认分配方式由 **采购参数 → 默认分配方式** 配置（系统参数 `System.RFQ.DefaultAssignMethod`），出厂默认 **采报优先（5）**。同页勾选 **允许指定采购**（`System.RFQ.AllowDesignatedPurchaser`，默认关闭）后，创建/编辑下拉才会出现指定采购。
...
| **4** | **指定采购** | **采购参数勾选「允许指定采购」后可选（下拉最后）** | **`DesignatedPurchaserAssignStrategy`**（详见 [指定采购-分配方式](./需求/指定采购-分配方式-设计与实现.md)） |
...
| 可选值 | 采报优先（5）、条目轮询（2）、品牌轮询（3）；**不含**指定采购 |
| 默认值 | **5（采报优先）** |
| 允许指定采购 | 同页勾选；默认关闭。开启后创建/编辑「分配方式」下拉才出现指定采购 |
| 作用范围 | 默认项仅影响 **新建需求页** 打开时的下拉选中；用户仍可改选（指定采购须勾选已开） |
| 不影响 | 已保存需求的 `assign_method`；编辑页加载已有需求的分配方式 |

系统参数：`System.RFQ.DefaultAssignMethod`（`ValueString` 存 2/3/5）、`System.RFQ.AllowDesignatedPurchaser`（Boolean，默认 false）。细则见 [采购参数-默认分配方式-设计与实现](./采购/采购参数-默认分配方式-设计与实现.md)、[指定采购-分配方式](./需求/指定采购-分配方式-设计与实现.md)。
...
| `sysparam` | `System.RFQ.DefaultAssignMethod`、`System.RFQ.AllowDesignatedPurchaser`、`System.RFQ.RoundRobinAssigneeCount`、报价员池相关参数 |
...
| 用途 | 方法 | 路径 | 权限 |
|------|------|------|------|
| 管理端读写默认分配方式与「允许指定采购」 | GET/PUT | `/api/v1/purchase-params/default-assign-method` | `system.params.purchase.default-assign-method.read` / `.write` |
| 新建/编辑需求页读取默认与开关 | GET | `/api/v1/rfqs/default-assign-method` | `rfq.create` 或 `rfq.write` |
...
| 新建/编辑需求默认项与「允许指定采购」 | `CRM.Web/src/views/RFQ/RFQCreate.vue`（`resolveDefaultAssignMethod` → 默认选中 + `allowDesignatedPurchaser`） |
...
| `RfqDefaultAssignMethodRulesTests` | 默认分配方式合法值（不含 4） |
| `DesignatedPurchaserAssignStrategyTests` | 指定采购只写槽位 1、不在池拒绝、沿用原人可出池 |
| `RfqDesignatedPurchaserRulesTests` / `RFQServiceTests` | 未开启指定采购时创建/改方式拒绝 |
...
| [指定采购-分配方式-设计与实现](./需求/指定采购-分配方式-设计与实现.md) | 采购参数「允许指定采购」开启后，创建/编辑页可选 assign_method=4 |
| [采购参数-默认分配方式-设计与实现](./采购/采购参数-默认分配方式-设计与实现.md) | 默认项与勾选的采购参数页 |
| [业务规则总览](./业务规则总览.md) §3 需求（RFQ） | 条目/品牌/采报优先/指定采购规则索引 |
...
| 1.1 | 2026-08-26 | 指定采购（4）改为创建页可选，链到专题设计 |
| 1.2 | 2026-08-26 | 指定采购须采购参数勾选「允许指定采购」后才出现在创建/编辑下拉 |
| 1.3 | 2026-08-26 | §3.7 / API / 参数表与采购参数页设计对齐 |

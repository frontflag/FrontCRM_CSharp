...
2. **文字列**（纵向多块）：
   ...
   - **第三行 `title-tags-row`**（可选）：**客户详情**等主数据页在标题组内单独一行放标签 + 「添加标签」。**单据类（需求 RFQ）** 已将标签 chip 与「± 标签」按钮放在 **`page-title-with-icons` 标题行**（收藏之后），详见 [业务详情页面规范 §3.5](./业务详情页面规范.md#35-主表标签区需求详情参考)。
...
- 单据类详情（如销售/采购订单、需求 RFQ）CaptionBar 扩展以 [业务详情页面规范](./业务详情页面规范.md) 为准：
  - **标题行：** `{前缀} {单号}`、收藏、主表标签（可选）、来源；
  - **副标题行：** 主单状态（**`el-tag effect="dark"`，与同模块列表「状态」列样式一致**）、明细统计 pill；
  - **右侧：** 关闭/取消（**无边框 ghost**，§3.6.1）→ 常规辅助如刷新（`btn-secondary` 线框，可选）→ 编辑（`btn-primary`）→ 更多。
- 单号格式与是否在基础信息重复展示，见 [业务详情页面规范 §3.2.1](./业务详情页面规范.md#321-主标题文案格式需求类)。

---

*文档版本：与客户详情页当前实现对齐；单据类 CaptionBar 扩展以 [业务详情页面规范](./业务详情页面规范.md) 为准。*

...
均应按本规范实现。**参考实现**：采购订单报表（路由 `purchase-orders/:id/report`）。采购订单 **V2** 见 [报表规范-采购订单-V2](./报表规范-采购订单-V2.md)；装箱单 **V2**（竖版 + 横版）见 [报表规范-装箱单-V2](./报表规范-装箱单-V2.md)。**不要**与各自 V1 三租户皮肤混用。
...
- **贴满 A4**：`renderElementToPdfBlob` 将报表节点隔离到视口左上角再截图，再按宽度贴满 jsPDF A4（210mm）。**不要**把 `html2canvas` 的 `windowWidth` 设成稿宽（会把居中预览连灰底一起缩小成「纸中纸」）；也**不要**再叠加 10mm 页边距。超高内容按整页宽度切片翻页。  

...
FrontCRM 采购明细 `FinancePaymentStatus`（0/1/2）按 **已核销合计 vs 行采购总额**，与扩展表付款进度一致：

```text
采购总额 = round(qty × cost)
已付款合计 = 有效付款单（排除取消、审核失败、已删除）明细 VerificationDone 之和

IF 已付款合计 == 0                  -> 未付款 (0)
IF 0 < 已付款合计 < 采购总额        -> 部分付款 (1)
IF 已付款合计 >= 采购总额          -> 付款完成 (2)
```

已付款合计 **不含** 在途未付的请款（已申请但尚未核销）。申请付款：已付款合计 < 采购总额即可继续申请；可申请金额 = 采购总额 − 已申请，请款已满时按钮仍灰。

> EBS 对照（只读）：[请款付款IMP](../../../EBS/财务/请款付款IMP.md) 按已付 vs **请款金额**。FrontCRM 已改为 vs **行采购总额**，以便分批请款。

---

## 四、状态流转关系

> **FrontCRM 实现**（状态值、编辑请款/付款执行拆分、权限矩阵）以 [付款单状态与操作权限](../../System/财务/付款单状态与操作权限.md) 为准。本节保留业务语义说明；下图已修正历史笔误（审核失败 `-1`、付款完成 `100`、取消 `-2`）。

### 4.1 付款单状态 (FinancePayment.Status)

| 值 | 枚举 | 说明 |
|----|------|------|
| 1 | New | 新建 |
| 2 | PendingAudit | 待审核 |
| 10 | Approved | 审核通过，待付款 |
| 100 | Completed | 付款完成 |
| -1 | AuditFailed | 审核失败 |
| -2 | Cancelled | 已取消 |
...
| 流转路径 | 触发条件 | 操作人 |
|----------|----------|--------|
| New → PendingAudit | 用户点击「提交审批」 | 采购员/财务（`purchase-order.write` 或 `finance-payment.write`） |
| PendingAudit → Approved | 审批中心通过 | 财务审批 |
| PendingAudit → AuditFailed | 审批中心驳回 | 财务审批 |
| AuditFailed → New | 编辑请款保存 | 采购员/财务 |
| Approved → New | **撤回**（清空付款执行信息） | 创建人或 `finance-payment.write` |
| Approved → Completed | 保存付款执行后「付款完成」 | 财务（`finance-payment.write`） |
| New/PendingAudit → Cancelled | 取消 | 财务（`finance-payment.write`） |
...
| 类型 | 允许状态 | 说明 |
|------|----------|------|
| 编辑请款 | 1、-1 | 改供应商账户、费用、明细请款额；不可增删 PO 明细行 |
| 付款执行 | 10 | 改公司付款银行、付款日、流水号、水单附件 |
...
| `付款完成状态 = 已付金额 >= 采购总额` | 采购明细付款状态（FrontCRM：已核销 vs 行 `qty×cost`，不是 vs 已请款） |

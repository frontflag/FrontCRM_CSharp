...
> **状态**：一期已落地（2026-06）  
> **范围**：统计分析模块 — **采购方向**（对齐 [`数据看板总要求.md`](./数据看板总要求.md) §5.2）  
> **总要求**：须符合 [`数据看板总要求.md`](./数据看板总要求.md)（角色权限、四类呈现、统计维度、跨域架构）  
> **对照模板**：[`销售统计.md`](./销售统计.md)（三层 Tab、API 分层、Scope 校验模式一致）  
> **列表页看板**（`/purchase-orders` 列表内「看板」按钮）：见 [`订单列表统计看板-设计与实现.md`](./订单列表统计看板-设计与实现.md) §5.2 — **已落地**。  
> **明细列表看板**（`/purchase-order-items`）：见 [`采购订单明细列表看板-设计与实现.md`](./采购订单明细列表看板-设计与实现.md) — **已落地**。
...
| 采购订单金额（已付款） | 金额数 | 静、趋 | ✅ | Σ `payment_amount_finish`（extend） |
...
| 指标 | 维度 | 呈现 | 一期 | 数据源 |
| --- | --- | --- | --- | --- |
| 应付款金额 | 金额数 | 待 | ✅ | Σ `payment_amount_not`（活跃 PO 明细） |
...
| 外币 | 跨币别汇总统一 **USD**（`convert_total` / `convert_price`） | 币别饼图按原币分桶展示美元额或 Mask 时订单数 |

**一期产品文案建议**：「采购统计分析（采购单按创建日、报价按明细/报价日；美元额 convert_total）」。
...
| 端点 | 说明 |
| --- | --- |
| `GET /api/v1/analytics/purchase/dashboard` | `scopeContext` + `snapshot` + `todo` + `rankings` |
| `GET /api/v1/analytics/purchase/trends` | `trend[]` |
| `GET /api/v1/analytics/purchase/breakdowns` | `breakdown[]` |
...
- 布局顺序：Tab → 待办 KPI → 静态 KPI → 趋势 + 分解饼图 → 排行。
- 个人层：`canSelectPurchaseUser === true` 时展示采购员下拉；选项来自 `scopeContext.allowedPurchaseUsers`。
- 钻取：`purchaseAnalyticsDrill.ts` → 报价列表、采购单列表、付款列表等（带 query 筛选）。
...
| 项 | 路径 |
| --- | --- |
| 页面 | `CRM.Web/src/views/Reports/PurchaseAnalyticsPage.vue` |
| API 客户端 | `CRM.Web/src/api/analytics/purchase.ts` |
...
| i18n | `purchaseAnalytics.*`（`zh-CN.ts` / `en-US.ts`） |
...
| **采购订单明细列表看板** | `document/IMP/数据看板/采购订单明细列表看板-设计与实现.md`；API `/api/v1/purchase-orders/items/analytics/*` |

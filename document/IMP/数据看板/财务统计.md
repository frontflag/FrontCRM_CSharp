...
| 汇率 / 折算 | **产品主口径**：销/采订单明细 `convert_price`（折算美金单价）与订单头 `convert_total`（折算美金金额）；延伸金额用 `FromExtend`（`local × convert_price / price`）。**回落**：价比不可用时才用 `ExchangeRateToUsdConverter` + 查询日 `FinanceExchangeRateSetting` |
...
- **已付款下钻（原币「查看」）**：仅 **财务模式 + 公司视角**、未脱敏、且有付款列表读权限时，各 `byCurrency` 行显示「查看」。整张已付款卡不可点。跳转 `/finance/payments`：`status=100`、`paymentCurrency`（付款单头币别，列表筛选项不展示）、`startDate`/`endDate` = KPI 窗口「区间起～截至」（`dateFrom` + `dateTo`/`asOfDate`，**不是**日期范围控件末端）。部门/个人视角、销/采归属模式不显示「查看」。  
- **已收款下钻（原币「查看」）**：同权限门控（收款列表读）。整张已收款卡不可点。跳转 `/finance/receipts`：`status=3`、`receiptCurrency`（收款单头币别，筛选项不展示）、`receiptDateFrom`/`receiptDateTo` = KPI 窗口（筛 **ReceiptDate**）。**不**占用列表 `startDate`/`endDate`（创建时间）。进项/销项发票卡仍整卡下钻。  
...
**产品确认（取代「一期默认查询日财务汇率」）**：折算美金以 **订单历史折算价** 为准——销售/采购订单明细的 `convert_price`（折算美金单价）与订单头 `convert_total`（折算美金金额，= Σ 行 `qty × convert_price`）。台账/extend 上的原币余额按关联订单行价比折算，反映成交当时汇率，而非打开看板当天的财务参数汇率。

| 数据源 | 原币 `amount` | 折 USD |
| --- | --- | --- |
| PO 明细 extend（款/进项票） | PO 头 `currency` + extend 金额字段 | **`FromExtend`**：PO 行 `convert_price`（USD 单价）× 比例，即 `extend_amount × convert_price / price`（与 `qty × convert_price` 同族） |
| SO 明细 extend（款/销项票） | SO 头 `currency` + extend 金额 | 同上，用 SO 行 `convert_price` |
| `finance_receivable` | `currency` + `amount` / `verified_*` | 原币字段直取；经 `sell_order_item_id` 挂 SO 行，`FromExtend`：`local × convert_price / price`；价比为 0 时回落查询日财务汇率 |
| `financepayment` / `financereceipt` | `payment_currency` / `receipt_currency` + 头/明细金额 | **已付款**：付款明细挂 PO 行 `FromExtend`（`payment_amount × convert_price / cost`）；无明细/无价比回落查询日汇率。**已收款**：明细优先 `receipt_convert_amount`；否则挂 SO 行 `FromExtend` |
| 进/销项发票 | 关联 PO/SO 币别；发票头/明细金额 | **已开进项**：明细经入库挂 PO 行 `FromExtend`；**已开销项**：明细经出库扩展挂 SO 行 `FromExtend`；无法追溯时回落查询日汇率 |
...
| 已收款 | `receivedAmount` | `finance_receivable` + FinanceReceipt | 存量/累计：Σ `finance_receivable.verified_done`（与 extend `receipt_amount_finish` 同源）；区间趋势按 **已确认** 收款单（`Status=3`）的 `receipt_date`。收款主状态见 [收款单状态-设计与实现](../../System/财务/收款单状态-设计与实现.md) |
...
| 折 USD | **主口径** SO 行 `convert_price/price`（`FromExtend`）；仅价比不可用时回落查询日财务汇率 |
...
| 口径 | 数据源 | 含义 |
| --- | --- | --- |
| 销售分析 · 应收 todo | `finance_receivable.verified_to_be` | 与应收款列表看板「待核销应收款」同源 |
| 财务分析 · 应收 todo | `finance_receivable.verified_to_be` | 与列表看板「待核销应收款」同源 |

销售待办与财务待办：同为待核销余额；销售另叠销售分析公司/部门/个人透镜，数字可因透镜不同而不等。
...
| `maskAmounts` | 金额脱敏 |
| `exchangeRateHint` | 如「美元折算优先销售/采购订单行折算单价（convert_price），缺省时用查询日财务参数汇率」 |
...
- 钻取：已付款 / 已收款改为原币行「查看」（见 §3.2）；进项/销项发票卡仍整卡映射财务列表路由。
...
| 2026-08-11 | 已付款/已收款趋势单位文案统一为「单位：折算美元（USD）」；KPI 小标题「折算美元」 |
| 2026-08-11 | `finance_receivable` 折 USD 改为优先 SO 行 `convert_price/price`（`FromExtend`） |
| 2026-08-11 | §4.2 产品确认：折算美金主口径改为订单 `convert_total`/`convert_price`（历史成交），取代「一期默认查询日财务汇率」 |
| 2026-08-11 | 已完成四项（已付/已收/已开进销项）与趋势：明细挂订单行 `FromExtend` / 收款 `receipt_convert_amount` |
| 2026-09-01 | 已付款：财务公司视角原币行「查看」下钻付款列表（`paymentCurrency` 隐藏筛选）；整卡不可点 |
| 2026-09-01 | 已收款：财务公司视角原币行「查看」下钻收款列表（隐藏 `receiptCurrency` + `receiptDateFrom/To`，不改创建时间筛选）；整卡不可点 |

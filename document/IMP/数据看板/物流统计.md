...
- 页头副标题：「在库存量截至查询日；库龄按入库日；金额 USD 采购成本」
...
**全局约束**：下列 **全部指标**（含 snapshot、todo、breakdown、customer-matrix、trends）均受 **库存类型**（§3.0）与 **viewLevel 透镜**（§5.4）约束，二者正交。
...
| 能力 | 一期 | 说明 |
| --- | --- | --- |
| 入库 PCS 趋势（按 `stock_in_date`） | ✅ | `GET .../trends` → `stockInQty`；受 `inventoryType` + `dateFrom/dateTo` + `groupBy` 约束 |
...
| --- | --- |
| 行在库数量 | `qty_repertory` |
| 行在库金额（USD） | `qty_repertory × purchase_price_usd` |
...
| `GET /api/v1/analytics/logistics/trends` | 入库/待入库趋势 |
...
- **钻取**（`logisticsAnalyticsDrill.ts`）：
  ...
  - **已知限制**：库存明细列表 API **暂无 `stockType` 筛选**，钻取时**未**携带 `inventoryType` 对应 `stock_type`（用户需在列表侧自行过滤，或待二期补 API）。

...
**时间语义**：期间流量，用 `dateFrom`～`trendDateTo`（与入库数量趋势同窗）。设计见 [物流分析-出入库概况-设计与实现](../../System/数据分析/物流分析-出入库概况-设计与实现.md)。  
...
| 能力 | 一期 | 说明 |
| --- | --- | --- |
| 入库 PCS 趋势（按 `stock_in_date`） | ✅ | `GET .../trends` → `stockInQty`；受 `inventoryType` + `dateFrom`/`trendDateTo` + `groupBy` 约束 |

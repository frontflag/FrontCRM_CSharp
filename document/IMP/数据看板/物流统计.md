...
| 功能权限 | 物流分析页：`analytics-logistics.read` **或** `inventory.read`。Controller 另允许 `purchase-order.read` / `sales-order.read`，供控制台业务总览「在库」调用（库存明细列表不要求 `inventory.read`） |
...
| 指标 | 维度 | 呈现 | 一期 | 说明 |
| --- | --- | --- | --- | --- |
| 入库金额 | 金额 | 静 | ✅ | 趋势区间内已过账采购入库；USD = `qty × PurchasePriceUsd`；原币「查看」仅物流公司视角 + 库存类型全部 |
| 出库金额 | 金额 | 静 | ✅ | 趋势区间内出库完成的销售出库；USD = `qty × SalesPriceUsd`；**仅状态 4**；原币「查看」仅物流公司视角 + 库存类型全部 |
...
| 项 | 说明 |
| --- | --- |
| Controller | `[RequireAnyPermission("analytics-logistics.read", "inventory.read", "purchase-order.read", "sales-order.read")]` |
| 路由 | `routes.ts` → `permissions: ['analytics-logistics.read', 'inventory.read']`（物流分析页仍二选一） |
| 菜单 | `AppLayout.vue`：同上二选一即可见「物流分析」 |
| 控制台在库 | 销售桌面：`DashboardOverviewPanel` 调本 Controller 取折算美金。采购桌面改为库存明细列表 `total`（条数），不走本接口 |
...
- **钻取**（`logisticsAnalyticsDrill.ts`）：
  ...
  - 出入库概况入库金额：非整卡；原币「查看」→ `/inventory/stock-in` + `stockInType=10` + 隐藏 `status=2` + 隐藏 `itemCurrency` + 入库日区间（仅物流公司视角且库存类型全部）；
  - 出入库概况出库金额：非整卡；原币「查看」→ `/inventory/stock-out/items` + `stockOutType=10` + 可见 `status=4` + 隐藏 `salesCurrency` + 出库日区间（仅物流公司视角且库存类型全部）；
  - **已知限制**：库存明细列表 API **暂无 `stockType` 筛选**，钻取时**未**携带 `inventoryType` 对应 `stock_type`（用户需在列表侧自行过滤，或待二期补 API）。入库/出库金额「查看」在客单/采购库存下不展示（列表同样无 `stock_type`）。
...
| 库存明细列表看板 | [`库存明细列表看板-设计与实现.md`](./库存明细列表看板-设计与实现.md) |
...
| 日期 | 说明 |
| --- | --- |
| 2026-09-01 | 出库金额原币「查看」下钻：筛选栏显示出库完成 `status=4`，隐藏销售原币 `salesCurrency`；仅物流公司视角 + 库存类型全部 |
| 2026-09-01 | 入库金额原币「查看」下钻：隐藏已过账 `status=2` 与明细原币 `itemCurrency`；仅物流公司视角 + 库存类型全部 |
| 2026-06-03 | 初稿 |
...
| 2026-09-03 | 库存明细列表嵌入看板（跟筛选、原币金额、周转天数、呆滞数量；见 `库存明细列表看板-设计与实现.md`） |
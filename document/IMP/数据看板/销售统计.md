...
| 层 | 设计 |
| --- | --- |
| API | `GET /api/v1/analytics/sales/dashboard` — `scopeContext` + **静态** `snapshot` + **待办** `todo` + 排行（不含客户 Top）<br>`GET /api/v1/analytics/sales/trends` — **趋势** `trend[]`<br>`GET /api/v1/analytics/sales/breakdowns` — **分解** `breakdown[]`（环节、币别等）<br>`GET /api/v1/analytics/sales/customer` — **客户 Tab**<br>`GET /api/v1/analytics/sales/order-items/{dashboard\|trends\|breakdowns\|rankings}` — **订单 Tab**（`reportApproved`）<br>`GET /api/v1/analytics/sales/rfq-items/{dashboard\|trends\|breakdowns\|rankings}` — **需求 Tab**（`reportScope`，排除主单已取消） |

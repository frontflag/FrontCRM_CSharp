...
```
报表分析（/reports）
├── 销售分析（/reports/sales）        ← 一期
├── 采购分析（/reports/purchase）     ← 后续
├── 库存分析（/reports/inventory）    ← 后续
└── 财务分析（/reports/finance）      ← 后续
```

- 将 `AppLayout` 中「报表分析」由占位改为可进入子模块；一期仅开放「销售分析」。
...
| 需求客户数 | 客户数 | 静 | ✅ | `rfq.customer_id` 去重 |
| 需求→销售订单转化比率 | 比率 | 静、趋 | ✅ | 已有 `sellorderitem` 关联需求行的 RFQ 明细 / 同期需求明细总数 |
...
| 销售订单金额（成单已审核） | 金额数 | 静、趋 | ✅ | `status >= Approved` 的 `convert_total` 汇总 |
| 销售订单金额（已出库） | 金额数 | 静、趋 | 二期 | extend 出库量×单价 或 `stock_out` 已出库额 |
| 销售订单金额（已收款） | 金额数 | 静、趋 | 二期 | `receipt_amount_finish` / 财务收款核销 |

### 3.3 销售订单 — 分解

| 指标 | 呈现 | 一期 | 说明 |
| --- | --- | --- | --- |
| 各环节执行状况比例 | 分 | ✅ | 饼图：主状态 + extend 进度（采购/入库/出库/收款/开票）分项占比 |
| 币别金额构成 | 分 | ✅ | 饼图：RMB vs 外币（或按 `currency` 分桶） |
...
| 待开票金额 | 金额数 | 待 | 二期 | `invoice_amount_not` |
...
| 决策点 | **一期推荐默认** | 备选（二期） |
| --- | --- | --- |
| 金额口径 | **下单额**：`sellorder.convert_total`（本位币） | 出库额、已收款额、已开票额 |
...
| 外币 | 展示层用 `convert_total`；可选「原币分组」副视图 | 统一折 USD |

**一期产品文案建议**：「销售统计分析（按订单创建、本位币下单额）」— 与列表 `total` / `convert_total` 一致，便于业务理解。
...
3. **层级校验**：`SalesAnalyticsScopeValidator`（Core）根据 `UserPermissionSummaryDto` 判断 `viewLevel` / `departmentId` / `salesUserId` 合法性；非法返回 **403**。
...
├──────────────────────────────────────────────────────────────────┤
│ scopeContext：当前可见「商务部及下级」· 部门视图 · 下单额(本位币)    │
├──────────┬──────────┬──────────┬──────────┤
│ KPI …    │          │          │          │
├──────────────────────────────────────────────────────────────────┤
...
| # | 问题 | 方案建议 |
| --- | --- | --- |
| 1 | 一期金额口径 | 默认「订单创建日 + 本位币下单额（`convert_total`）」 |
...
| 菜单占位 | `CRM.Web/src/layouts/AppLayout.vue`（`reportAnalytics`） |

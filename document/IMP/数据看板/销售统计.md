...
> **状态**：一期已落地；**二期 M1 已落地**；**内容 Tab**（概况 / 客户 / 需求 / 订单）已落地（2026-07）  
> **范围**：统计分析模块 — **销售方向**（一期 + 二期）  
> **总要求**：须符合 [`数据看板总要求.md`](./数据看板总要求.md)（角色权限、四类呈现、统计维度、跨域架构、§3.6 内容 Tab）  
> **关联**：侧栏「报表分析」→ 销售分析 `/reports/sales` 已开放。  
> **列表页看板**（`/sales-orders` 列表内「看板」按钮，筛选联动）：见 [`订单列表统计看板-设计与实现.md`](./订单列表统计看板-设计与实现.md) §5.1 — **已落地**，与本文 `/reports/sales` **独立**（客户维面板已迁至本文「客户」Tab，见 §3.6）。  
> **明细列表看板**（`/sales-order-items`）：见 [`销售订单明细列表看板-设计与实现.md`](./销售订单明细列表看板-设计与实现.md) — **已落地**（订单 Tab `reportApproved`）。  
> **需求明细列表看板**（`/rfq-items`）：见 [`需求列表统计看板-设计与实现.md`](./需求列表统计看板-设计与实现.md) — **已落地**（需求 Tab `reportScope`）。  
> **概况分解 · 出库进度「口径 / 详情」**：见 [`销售分析-出库进度详情与口径-设计与实现.md`](../../System/数据分析/销售分析-出库进度详情与口径-设计与实现.md) — **已落地**。  
> **概况待办「口径 / 详情」**：见 [`销售分析-待办口径与详情-设计与实现.md`](../../System/数据分析/销售分析-待办口径与详情-设计与实现.md) — **已落地**（详情为列表新标签下钻）。
...
| 销售订单金额（已出库） | 金额数 | 静、趋 | ✅ | 原币 Σ `qty×price` 分档；美金 Σ `qty×convert_price` |
| 销售订单金额（已收款） | 金额数 | 静、趋 | ✅ | 原币 Σ `receipt_amount_finish` 分档；美金 FromExtend |
...
| 出库进度（明细行） | 分 | ✅ | 柱图；面板「口径」Tip（i18n，对齐帮助）+「详情」弹窗（信息栏视角/区间、可拖拽缩放）；`GET .../breakdowns/stock-out-progress/detail` |

### 3.4 销售订单 — 待办

| 指标 | 维度 | 呈现 | 一期 | 数据源 |
| --- | --- | --- | --- | --- |
| 待核销应收款 | 金额数 | 待 | ✅ | `finance_receivable` Σ `verified_to_be`；美金挂 SO 行 `FromExtend`（`convert_price/price`；对齐列表看板；总要求 §四） |
...
| viewLevel | 概况排行 | 呈现 |
| --- | --- | --- |
| 公司 | 部门销售额 Top10 | 表格（可下钻部门层） |
| 部门 | 部门内业务员 Top10 | 表格（可下钻个人层 / 双击进订单） |
| 个人 | （无） | 客户排行已迁至「客户」Tab |

### 3.6 客户 Tab（已从 `/sales-orders` 列表看板迁入）

| 指标 | 呈现 | API | 口径 |
| --- | --- | --- | --- |
| 成单客户数 | 静 | `GET .../analytics/sales/customer` | 与概况「销售客户数」同口径：周期内成单 `customer_id` 去重 |
| 复购客户数 | 静 | 同左 | 成单订单数 ≥ 2 的去重客户 |
| 客户类型 / 等级 / 行业 | 分 | 同左 | 成单 USD；Mask 时按订单数占比 |
| 客户 Top10（金额 / 成单数 / 复购订单数） | 表 | 同左 | 复购订单数 = max(0, 成单数−1)；可下钻订单列表 |

- 筛选：与概况共用公司/部门/个人 + 日期（**非**列表页筛选）。
- 加载：进入「客户」Tab 后再请求（lazy）。
- 前端：`SalesAnalyticsCustomerPanel.vue`；概况不再展示「客户贡献 Top10」。

### 3.7 订单 Tab（与 `/sales-order-items` 明细看板同实现）

| 呈现 | 内容 | API |
| --- | --- | --- |
| Tag | 「成单」 | — |
| 静 | 成单概览 / 利润 / 在库 / 应收 | `GET .../analytics/sales/order-items/dashboard` |
| 趋 | 成单订单数 / 明细数 / 金额（含 groupBy） | `.../order-items/trends` |
| 分 | 11 组分解（状态条图 + 进度/币别/品牌等饼图） | `.../order-items/breakdowns` |
| 表 | 6 张 Top10 | `.../order-items/rankings` |

- **数据集**：`dataset=reportApproved`（报表透镜 + 日期 + 成单）；与列表看板 `listFilter` 共用 `ISalesOrderItemLineListQuery`。
- 筛选：与概况共用公司/部门/个人 + 日期；**含趋势与 groupBy**。可选 `customerId` 精确锁定客户（客户详情「业务画像」用；销售分析页不传）。
- 加载：进入「订单」Tab 后再请求（lazy）。
- 前端：复用 `SalesOrderItemListBoard.vue`（`mode=report`）。
- 对照：[`销售订单明细列表看板-设计与实现.md`](./销售订单明细列表看板-设计与实现.md)。

### 3.8 需求 Tab（与 `/rfq-items` 明细看板同实现）

| 呈现 | 内容 | API |
| --- | --- | --- |
| Tag | 「报表」 | — |
| 静 | 发布客户 / 复询 / 需求数 / 明细 / 成单 / 成单率 | `GET .../analytics/sales/rfq-items/dashboard` |
| 趋 | 客户数 / 需求数 / 明细数（含 groupBy） | `.../rfq-items/trends` |
| 分 | 8 组分解 | `.../rfq-items/breakdowns` |
| 表 | 6 张 Top10 | `.../rfq-items/rankings` |

- **数据集**：`dataset=reportScope`（报表透镜 + 主单创建日 + **排除主单已取消**）；与列表看板 `listFilter` 共用 `IRfqItemListQuery`。
- 筛选：与概况共用公司/部门/个人 + 日期；可选 `customerId` 精确锁定客户（客户详情「业务画像」用）。
- 加载：进入「需求」Tab 后再请求（lazy）。
- 前端：复用 `RfqItemListBoard.vue`（`mode=report`）。
- 对照：[`需求列表统计看板-设计与实现.md`](./需求列表统计看板-设计与实现.md)。
...
| 外币 / 折算美金 | **产品主口径**：销售订单明细 `convert_price`（折算美金单价）与订单头 `convert_total`（折算美金金额）。出库/收款/待开票/待核销应收等延伸金额用 `FromExtend`（`local × convert_price / price`）。**不以查询日财务汇率为主口径**；仅价比不可用时回落查询日财务参数汇率 | — |

**一期产品文案建议**：「销售统计分析（按订单创建、美元下单额 convert_total）」— 列表汇总与看板均使用订单历史折算价（`convert_total` / `convert_price`），原币行金额见订单 `total` / 明细 `price`。
...
### 5.6 前端交互（Scope Tab + 内容 Tab）

```
┌──────────────────────────────────────────────────────────────────┐
│ 销售分析  [公司] [部门] [个人]   ← Scope Tab（SaleDataScope）      │
│ 日期 […]  [部门▼] [业务员▼]                                      │
├──────────────────────────────────────────────────────────────────┤
│ scopeContext 提示条                                              │
├─ 内容 Tab：[概况] [客户] [需求] [订单]  ← lazy 子面板 ────────────┤
│ 概况：待办 → 静态 KPI → 趋势/分解 → 排行（不含客户 Top）           │
│ 客户 / 需求 / 订单：见 §3.6–3.8                                   │
└──────────────────────────────────────────────────────────────────┘
```
...
| 交互 | 规则 |
| --- | --- |
| 默认 Scope Tab | Scope=1 → **个人**；Scope=2/3 → **部门**；Scope=0 / Admin → **公司** |
| 默认内容 Tab | **概况** |
| 公司 Tab 文案 | Scope=0 显示「公司」；Scope=2/3 显示「可见范围」 |
| 部门下拉 | 仅 Scope 0/3 展示；Scope=2 固定本部门，不展示下拉 |
| 越权 Tab | 不渲染或 `disabled` + tooltip「当前数据范围不可用」 |
| 钻取 | 部门行 → 切到部门层并选中该部门；业务员行 → 切到个人层；客户 Top → 订单列表 |

### 5.7 一期 MVP 与三层的范围对齐

| 层级 | 概况排行（客户 Top 见「客户」Tab） |
| --- | --- |
| 个人 | ✅ KPI + 趋势；概况无排行 |
| 部门 | ✅ KPI + 部门内业务员 Top10（Scope 允许时） |
...
| 层 | 设计 |
| --- | --- |
| API | `GET /api/v1/analytics/sales/dashboard` — `scopeContext` + **静态** `snapshot` + **待办** `todo` + 排行（不含客户 Top）<br>`GET /api/v1/analytics/sales/trends` — **趋势** `trend[]`<br>`GET /api/v1/analytics/sales/breakdowns` — **分解** `breakdown[]`（环节、币别等）<br>`GET /api/v1/analytics/sales/breakdowns/stock-out-progress/detail` — **出库进度详情**<br>`GET /api/v1/analytics/sales/customer` — **客户 Tab**<br>`GET /api/v1/analytics/sales/order-items/{dashboard\|trends\|breakdowns\|rankings}` — **订单 Tab**（`reportApproved`）<br>`GET /api/v1/analytics/sales/rfq-items/{dashboard\|trends\|breakdowns\|rankings}` — **需求 Tab**（`reportScope`，排除主单已取消） |
...
```json
{
  ...
  "todo": {
    "receivableAmount": {
      "totalUsd": 0,
      "byCurrency": [{ "currency": 1, "currencyLabel": "RMB", "amount": 0 }]
    },
    "pendingStockOutItemCount": 0,
    "pendingInvoiceAmount": { "totalUsd": 0, "byCurrency": [] }
  },
  "rankings": { }
}
```
...
### 6.2 前端（对齐总要求 §3.5–3.6 布局）

| 项 | 设计 |
| --- | --- |
| 页面 | `CRM.Web/src/views/Reports/SalesAnalyticsPage.vue`（`contentTab`：overview / customer / rfq / order） |
| 客户面板 | `CRM.Web/src/components/Analytics/SalesAnalyticsCustomerPanel.vue` |
| 需求看板 | `CRM.Web/src/views/RFQ/RfqItemListBoard.vue`（`mode=list\|report`） |
| 订单明细看板 | `CRM.Web/src/views/RFQ/SalesOrderItemListBoard.vue`（`mode=list\|report`） |
| API 客户端 | `CRM.Web/src/api/analytics/sales.ts`（含 `getCustomer` / `getOrderItems*` / `getRfqItems*`） |
| 钻取 | `CRM.Web/src/utils/salesAnalyticsDrill.ts`（或页面内联路由） |
| 路由 | `/reports/sales` |
| 布局顺序 | ① Scope Tab + 筛选 ② **内容 Tab** ③ 概况内：待办 → 静态 KPI → 趋势/分解 → 排行 |
| 组件 | `AnalyticsScopeTabs`、`AnalyticsKpiGrid`、`AnalyticsTrendChart`、`AnalyticsBreakdownChart` |
| i18n | `salesAnalytics.*`（含 `contentTabs` / `customerTab` / `rfqTab` / `orderTab`） |
| 钻取 | 待办/排行 → 销售订单、需求、应收列表（带 query 筛选） |
| 金额 KPI UI | 对齐总要求 [§3.8](./数据看板总要求.md) 与 [金额与币别范式 §5.2](../../PRD/规范/UI规范/金额与币别代码显示范式.md)：待办应收/待开票 `layout=split`（主金额 18px）；概况已出库/已收款主数字折算美金 +「查看本币」Tip；成单 `valueSuffix`「（折算美金）」；金额卡 `forceNewRow`；币别色仅金额尾缀 |
| 口径 Tip 标签 | 对齐总要求 [§3.9](./数据看板总要求.md)：KPI Tip 首行标签 = 帮助表第一列标题 **指标**（`definitionTip.metric`）；分解图 Tip 首行 = **图**（`definitionTip.chart`）；内容 i18n 与帮助行一致 |
...
| 2026-07-30 | §3.6 客户 Tab（自订单列表看板迁入）；概况不再展示客户 Top |
| 2026-07-30 | §3.7 订单 Tab：复用明细看板实现；API `/api/v1/analytics/sales/order-items/*`（`reportApproved`） |
| 2026-07-30 | §3.8 需求 Tab：复用 RFQ 明细看板；API `/api/v1/analytics/sales/rfq-items/*`（`reportScope`，排除主单已取消） |
| 2026-08-01 | §5.6 / §6.2 回写内容 Tab 前端结构与组件索引；对齐总要求 §3.6 |
| 2026-08-11 | §3.3 / §6.1：概况「出库进度」口径 Tip + 详情弹窗；交叉引用 System 设计文档 |
| 2026-08-11 | 分解条图同步展示行数/金额与百分比；总规见 [`数据看板总要求.md` §3.7](./数据看板总要求.md) |
| 2026-08-11 | 概况待办三卡「口径 / 详情」；详情新标签下钻列表 |
| 2026-08-11 | 待办应收/待开票：原币分档 + 折算美金（对齐应收款列表看板） |
| 2026-08-11 | §6.2：概况/待办金额 KPI 样式定稿（查看本币 Tip、split 18px、币别色仅尾缀、成单后缀、forceNewRow）；交叉引用总要求 §3.8 |
| 2026-08-11 | §6.2：口径 Tip 首行标签来源（KPI→帮助「指标」列 / 分解→「图」列）；交叉引用总要求 §3.9 |
| 2026-08-11 | 概况金额趋势单位文案改为「单位：折算美元（USD）」（`salesAnalytics.trendUnit.moneyCaption`） |
| 2026-08-11 | 应收趋势改为折算美金主数字（同已收款 FromExtend）；前端改 `$` + 折算美元单位；不再原币混加标本位币 |
| 2026-08-11 | 分解图 money 格式由 ¥ 改为 $（订单主状态/币别等本为 convert_total 折算美金） |
| 2026-08-11 | 待核销应收款美金改为优先 SO 行 `convert_price/price`（与列表看板/财务分析对齐） |

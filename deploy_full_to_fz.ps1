# Deploy FrontCRM — 仿真环境（FZ）服务器：前端 + 后端 构建并上传
# 默认目标：43.129.212.65（与生产 deploy_full_to_server.ps1 的 129.226.161.3 分离）
...
#   2) 将 frontcrm_deploy 目录「内容」同步到 FZ 服务器（逻辑与 deploy_full_to_server.ps1 相同）
#
# 若已本地构建好 frontcrm_deploy，可传 -SkipBuild 只上传。
#
# 用法（在项目根目录）：
#   .\deploy_full_to_fz.ps1
#   .\deploy_full_to_fz.ps1 -SkipBuild
#   .\deploy_full_to_fz.ps1 -ServerIP "43.129.212.65" -ServerUser "ubuntu"
...
try {
    ...
    if (-not [string]::IsNullOrWhiteSpace($selfPath) -and (Test-Path -LiteralPath $selfPath)) {
        ...
        if (-not $hasUtf8Bom) {
            Write-Host "ERROR: deploy_full_to_fz.ps1 must be UTF-8 with BOM." -ForegroundColor Red
...
# FZ 与 semicore 共用 production.semicore 构建；SkipBuild 时优先使用 frontcrm_deploy_semicore。
if ($DeployPackageName -eq 'frontcrm_deploy' -and $SkipBuild) {
    $tenantPackage = 'frontcrm_deploy_semicore'
...
Write-Host "FrontCRM full deploy -> FZ server $ServerIP (frontend + backend)" -ForegroundColor Green
Write-Host "============================================" -ForegroundColor Green
Write-Host ""
...
if (-not $AllowPasswordPrompt) {
    ...
    Write-Host "  For password upload: .\deploy_full_to_fz.ps1 -SkipBuild -AllowPasswordPrompt" -ForegroundColor Cyan
...
Write-Host ""
Write-Host "deploy_full_to_fz.ps1 done." -ForegroundColor Green

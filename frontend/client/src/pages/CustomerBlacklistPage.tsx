/**
 * CustomerBlacklistPage — FrontCRM Deep Quantum Theme
 * 客户黑名单管理：显示黑名单客户，支持移出黑名单恢复为正常状态
...
import {
  ArrowLeft, Ban, ShieldCheck, Loader2, Search, Building2, Calendar, User, FileText,
} from "lucide-react";
import { customerApi } from "@/lib/customerApi";

type BlacklistedCustomer = {
  id: string;
  customerCode: string;
  customerName: string;
  officialName?: string;
...
export default function CustomerBlacklistPage() {
  ...
  const loadData = async () => {
    setLoading(true);
    const res = await customerApi.getBlacklist(1, 100);
    setLoading(false);
    if (res.success && res.data) {
      const data = res.data as { items: BlacklistedCustomer[] };
  ...
  const handleRemove = async (id: string, name: string) => {
    setRemovingId(id);
    const res = await customerApi.removeBlacklist(id);
    setRemovingId(null);
    if (res.success) {
      ...
      <div className="max-w-5xl mx-auto px-4 py-6 space-y-5">
        ...
        {loading ? (
          ...
          <div className="text-center py-20 rounded-xl" style={{ background: "#0A1628", border: "1px solid rgba(0,212,255,0.1)" }}>
            <Ban size={40} className="mx-auto mb-3 opacity-20" style={{ color: "#C95745" }} />
            <p className="text-sm" style={{ color: "rgba(200,216,232,0.4)", fontFamily: "Noto Sans SC" }}>
              {search ? "没有匹配的黑名单客户" : "黑名单为空"}
            </p>
          </div>
        ) : (
            ...
            {filtered.map((c) => {
              ...
              return (
                <div
                  ...
                  style={{ background: "#0A1628", border: "1px solid rgba(201,87,69,0.25)" }}
                >
                  <div className="flex items-start justify-between gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-1.5">
                        ...
                        <span className="text-xs px-1.5 py-0.5 rounded flex items-center gap-1" style={{ background: "rgba(201,87,69,0.1)", color: "#C95745", border: "1px solid rgba(201,87,69,0.3)", fontFamily: "Noto Sans SC" }}>
                          <Ban size={10} />
                          黑名单
                        </span>
                      </div>
                      ...
                      <div className="mt-3 p-3 rounded-lg space-y-1.5" style={{ background: "#162233", border: "1px solid rgba(201,87,69,0.15)" }}>
                        {c.blackListAt && (
                          <div className="flex items-center gap-2 text-xs" style={{ color: "rgba(200,216,232,0.55)", fontFamily: "Noto Sans SC" }}>
                            ...
                            <span>加入时间：{new Date(c.blackListAt).toLocaleString("zh-CN")}</span>
                          </div>
                        )}
                        {c.blackListByUserName && (
                          <div className="flex items-center gap-2 text-xs" style={{ color: "rgba(200,216,232,0.55)", fontFamily: "Noto Sans SC" }}>
                            <User size={11} style={{ color: "#C95745" }} />
                            <span>操作人：{c.blackListByUserName}</span>
                          </div>
                        )}
                        {c.blackListReason && (
                          <div className="flex items-start gap-2 text-xs" style={{ color: "rgba(200,216,232,0.55)", fontFamily: "Noto Sans SC" }}>
                            ...
                            <span>黑名单理由：{c.blackListReason}</span>
                          </div>
                        )}
                      </div>
                    </div>
                    ...
                    {/* Right: Remove Button */}
                    <button
                      onClick={() => handleRemove(c.id, c.officialName || c.customerName)}

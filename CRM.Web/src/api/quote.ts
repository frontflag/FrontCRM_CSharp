...
function buildItemsFromQuoteCreateForm(form: Record<string, unknown>): Record<string, unknown>[] {
  ...
  for (const r of rows) {
    ...
    items.push({
      ...
      vendorLevel: form.vendorLevel != null && form.vendorLevel !== '' ? Number(form.vendorLevel) : undefined,
...
function mapQuoteEditSimpleToUpdate(body: Record<string, unknown>): Record<string, unknown> {
  ...
  const items = rawItems.map((it) => {
    ...
    vendorLevel: it.vendorLevel != null && it.vendorLevel !== '' ? Number(it.vendorLevel) : undefined,

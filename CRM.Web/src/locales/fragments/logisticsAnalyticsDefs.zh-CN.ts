export const logisticsAnalyticsDefsZh = {
  ...
  trend: {
    stockInQty: {
      chart: '入库数量趋势',
      dataSource: '入库单明细',
      text: '按日/周/月，把趋势区间拆成多个时段；每个点是该时段内已过账采购入库行的入库数量合计（与「入库金额」同窗同单据）。表示期间入库流量，不是在库存量。'
    },
    stockOutQty: {
      chart: '出库数量趋势',
      dataSource: '出库明细',
      text: '按日/周/月，把趋势区间拆成多个时段；每个点是该时段内出库完成的销售出库行数量合计（与「出库金额」同窗同单据）。不含准备出库，也不含报关/退货/报废/调拨出库。'
    }
  },
  matrix: {
    customer: {

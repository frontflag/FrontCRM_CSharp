...
import { financePaymentBankSelectedDisplayLabel } from '@/constants/financePaymentBankCurrencyType'
...
export function useFinancePaymentBankOptions() {
  ...
  function selectedPaymentBankName(vendorBankId: string): string {
    ...
    const hit = paymentBankOptions.value.find((b) => b.id === vendorBankId)
    return hit ? financePaymentBankSelectedDisplayLabel(hit) : vendorBankId
  }

<template>
  <div class="customer-home">
          ...
          <div class="customer-home__pill-wrap">
            <div class="customer-home__pill" role="search">
              ...
              <el-tooltip
                ...
                :content="t('customerHome.aiSearchNoPermission')"
                placement="bottom"
              >
                <span class="customer-home__pill-btn-wrap">
                  <button
                    type="button"
                    class="customer-home__pill-btn"
                    :disabled="!canAiIntelLookup || aiLoading"
                    @click="handleAiSearch"
                  >
                    {{ t('customerHome.aiSearch') }}
                  </button>
                </span>
              </el-tooltip>
              <button type="button" class="customer-home__pill-btn customer-home__pill-btn--secondary" @click="handleSearch">
                {{ t('customerHome.searchCustomers') }}
              </button>
              <button type="button" class="customer-home__pill-link" @click="goListPlain">{{ t('customerHome.goList') }}</button>
            </div>
            <p v-if="canAiIntelLookup" class="customer-home__ai-hint">
              {{ t('customerHome.aiSearchHint') }}
            </p>
          </div>
        </div>
    ...
    <div v-if="aiLoading" class="customer-home__ai-loading">
      <el-icon class="is-loading customer-home__ai-loading-icon"><Loading /></el-icon>
      <span>{{ t('customerHome.aiLoading', { seconds: aiLoadingSeconds }) }}</span>
    </div>
...
import { customerIntelApi } from '@/api/customerIntel'
import { AI_PERMISSION_ENTITY_PARSE_CUSTOMER, AI_PERMISSION_ENTITY_PARSE_CUSTOMER_BUSINESS_CARD, AI_PERMISSION_CUSTOMER_INTEL_LOOKUP } from '@/api/ai'
...
const canAiIntelLookup = computed(() => authStore.hasPermission(AI_PERMISSION_CUSTOMER_INTEL_LOOKUP))
...
async function handleAiSearch() {
  ...
  if (!companyName) {
    ElMessage.warning(t('customerHome.aiSearchNeedCompany'))
    return
  }
  ...
  try {
    ...
    if (report && typeof report === 'object' && !Array.isArray(report) && Object.keys(report).length > 0) {
      ...
      ElMessage.warning(t('customerHome.aiSearchEmpty'))
    }
  } catch (e: unknown) {
    aiResultData.value = null
    ElMessage.error(getApiErrorMessage(e, t('customerHome.aiSearchFailed')))
  } finally {
...
.customer-home__ai-loading-icon {
  font-size: 20px;
  color: $cyan-primary;
}

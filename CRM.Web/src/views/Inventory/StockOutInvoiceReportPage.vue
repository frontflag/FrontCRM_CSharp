...
import { resolveInvoiceLineAmounts, sumInvoiceLineAmounts, formatInvoiceMoney, resolveInvoiceTotalCurrency, applyCustomsInvoiceUsdPrices, resolveCustomsInvoiceStockOutType } from '@/utils/invoiceReportLines'
...
async function load() {
  ...
  try {
    ...
    // 报关装箱 Invoice 行与单价由后端按报关明细 / cost_usd 给出，前端不得再用折算美金价覆盖。
    const needsPackingDetail = Boolean(fromPacking) && lines.length === 0
    ...
    if (!fromPacking) {
      ...
      applyCustomsInvoiceUsdPrices(invoicePackingType, packingLines.value, null)
    }

...
import {
  inventoryOnHandListAnalyticsApi,
  type InventoryOnHandListAnalyticsBreakdownGroup,
  type InventoryOnHandListAnalyticsDashboard,
  type InventoryOnHandListAnalyticsQuery,
  type InventoryOnHandListAnalyticsRankingFacet,
  type InventoryOnHandListAnalyticsRankingRow,
  type InventoryOnHandListAnalyticsRankings,
...
import {
  ...
  buildStagnantStockItemDrillRoute,
  openInventoryBoardDrillInNewTab
...
const { def } = useAnalyticsDefinition('inventoryOnHandList.board')

const loading = ref(false)
const dashboard = ref<InventoryOnHandListAnalyticsDashboard | null>(null)
...
function tt(path: string, values?: Record<string, unknown>): string {
  return values
    ? t(`inventoryOnHandList.board.${path}`, values)
    : t(`inventoryOnHandList.board.${path}`)
}
...
function onKpiClick(key: string) {
  if (key !== 'stagnantLayers') return
  if (!authStore.hasPermission('inventory.read')) {
    ElMessage.warning(tt('drill.noPermission'))
    return
  }
  openInventoryBoardDrillInNewTab(router, buildStagnantStockItemDrillRoute(props.filters))
}
...
function onRankingRowClick(
  ...
  openInventoryBoardDrillInNewTab(
    ...
    buildRankingStockItemDrillRoute(
      props.filters,
      panel.key as 'customer' | 'salesUser' | 'material' | 'brand',
      row,
      effectiveRankingMetricMode.value,
      effectiveRankingMetricMode.value === 'amount' ? amountCurrencyKey.value : undefined,
      rankingTitle(panel)
    )
  )
}
...
const kpiItems = computed(() => {
  ...
  return [
    ...
    {
      key: 'weightedAvgAgeDays',
      label: tt('kpi.weightedAvgAgeDays'),
      value: formatAge(s.weightedAvgAgeDays),
      ...def('kpi.weightedAvgAgeDays')
    },
    {
      key: 'stagnantLayers',
      label: tt('kpi.stagnantLayers'),
      value: String(s.stagnantLayerCount),
      drillable: true,
      ...def('kpi.stagnantLayers')
    }
  ]
})
...
const TREND_AMOUNT_CURRENCIES = [
  { currencyKey: '1', currencyLabel: 'RMB' },
  { currencyKey: '2', currencyLabel: 'USD' }
] as const

const trendAmountSeries = computed(() =>
  TREND_AMOUNT_CURRENCIES.map((ccy) => ({
...
async function loadData() {
  ...
  try {
    ...
    const [dash, trendRows, breakdownRows, rankingRows] = await Promise.all([
      inventoryOnHandListAnalyticsApi.getDashboard(q),
      inventoryOnHandListAnalyticsApi.getTrends(q),
      inventoryOnHandListAnalyticsApi.getBreakdowns(q),
      inventoryOnHandListAnalyticsApi.getRankings(q)
    ])
...
<template>
  <div class="ioh-list-board" v-loading="loading">

    ...
    >
      ...
      >
        <section class="info-section write-off-panel write-off-panel--center" :style="centerPanelStyle">
          <div class="section-header">
            <div class="section-header__main">
              ...
              <span class="section-title">{{ t('purchaseInvoiceWriteOffWorkspace.invoicePanelTitle') }}</span>
              <span v-if="vendorId && invoices.length" class="section-count">{{ invoices.length }}</span>
            </div>
            <div
              class="section-header__layout"
              role="group"
              :aria-label="t('purchaseInvoiceWriteOffWorkspace.panelsLayoutGroup')"
            >
              <el-tooltip
                :content="t('purchaseInvoiceWriteOffWorkspace.panelsLayoutRow')"
                placement="top"
                :hide-after="0"
              >
                <el-button
              ...
              <el-tooltip
                :content="t('purchaseInvoiceWriteOffWorkspace.panelsLayoutColumn')"
                placement="top"
                :hide-after="0"
              >
                <el-button
          ...
          <div class="detail-panel-section-body write-off-panel__body">
            <div class="panel-info-bar">
              <span class="panel-info-bar__label">{{ t('purchaseInvoiceWriteOffWorkspace.currentVendorLabel') }}</span>
              <template v-if="!vendorSummary">
                <span class="panel-info-bar__hint panel-info-bar__hint--placeholder">
                  {{ t('purchaseInvoiceWriteOffWorkspace.currentVendorHint') }}
                </span>
              </template>
              <template v-else>
                <span class="panel-info-bar__customer">
                  <span class="panel-hint__value">{{ vendorNameZh }}</span>
                  <span v-if="vendorNameEn" class="panel-hint__sep"> / </span>
                  <span v-if="vendorNameEn" class="panel-hint__value">{{ vendorNameEn }}</span>
                </span>
            ...
            <div class="detail-items-table-wrap">
              <DetailListPanelEmpty
                v-if="!vendorId"
                size="medium"
                :description="t('purchaseInvoiceWriteOffWorkspace.selectVendorHint')"
              />
              <DetailListPanelEmpty
                v-else-if="!invoices.length"
                size="medium"
                :description="t('purchaseInvoiceWriteOffWorkspace.noInvoices')"
              />
              <el-table
                v-else
                :data="invoices"
              ...
              >
                <el-table-column
                  :label="t('purchaseInvoiceWriteOffWorkspace.colInvoiceCode')"
                  min-width="118"
                ...
                >
                  <template #default="{ row }">
                    <router-link
                      v-if="row.id"
                      class="link-text"
                      :to="{ name: 'FinancePurchaseInvoiceDetail', params: { id: row.id } }"
                      @click.stop
                    >
                ...
                <el-table-column :label="t('purchaseInvoiceWriteOffWorkspace.colInvoiceDate')" width="108">
                  <template #default="{ row }">{{ formatDate(row.invoiceDate) }}</template>
                </el-table-column>
                <el-table-column :label="t('purchaseInvoiceWriteOffWorkspace.colInvoiceType')" width="120">
                  <template #default="{ row }">{{ invoiceTypeLabelForRow(row) }}</template>
                </el-table-column>
                <el-table-column
                  :label="t('purchaseInvoiceWriteOffWorkspace.colInvoiceAmount')"
                  width="132"
                  align="right"
                  header-align="right"
                >
                ...
                <el-table-column
                  :label="t('purchaseInvoiceWriteOffWorkspace.colVerifiedDone')"
                  width="132"
                  align="right"
                  header-align="right"
                >
                  <template #default="{ row }">
                    <span class="finance-amount-emphasis finance-amount-emphasis--verified">
                      {{ formatAmountWithCurrency(row.verifiedDone, row.currency) }}
                    </span>
                  </template>
                </el-table-column>
                <el-table-column
                  :label="t('purchaseInvoiceWriteOffWorkspace.colVerifiedToBe')"
                  width="132"
                  align="right"
                  header-align="right"
                >
                  <template #default="{ row }">
                    <span class="finance-amount-emphasis finance-amount-emphasis--pending">
                      {{ formatAmountWithCurrency(row.verifiedToBe, row.currency) }}
                    </span>
                  </template>
                </el-table-column>
                <el-table-column
                ...
                >
                  ...
                  <template #default="{ row }">
                    <div class="customer-row-select-action" @click.stop>
                      ...
                      >
                        {{ t('purchaseInvoiceWriteOffWorkspace.selectRow') }}
                      </el-button>
                    </div>
                  </template>
                </el-table-column>
        ...
        <div
          ...
          :title="
            panelsLayout === 'column'
              ? t('purchaseInvoiceWriteOffWorkspace.dragCenterHeight')
              : t('purchaseInvoiceWriteOffWorkspace.dragCenterWidth')
          "
        ...
        <section class="info-section write-off-panel write-off-panel--right">
          <div class="section-header">
            <div class="section-header__main">
              <div class="section-dot section-dot--cyan"></div>
              <span class="section-title">{{ t('purchaseInvoiceWriteOffWorkspace.stockInPanelTitle') }}</span>
            </div>
          </div>
          <div class="detail-panel-section-body write-off-panel__body">
            <div v-if="vendorId" class="panel-info-bar panel-info-bar--with-total">
              <span class="panel-info-bar__main">
                <template v-if="selectedInvoice">
                  {{
                    t('purchaseInvoiceWriteOffWorkspace.selectedInvoiceInfoBarPrefix', {
                      invoiceCode: selectedInvoice.invoiceCode || selectedInvoice.invoiceNo || '—'
                    })
                  }}<span class="panel-info-bar__highlight-amount">{{
                    formatAmountWithCurrency(selectedInvoice.verifiedToBe, selectedInvoice.currency)
                  }}</span>
                </template>
                <template v-else>
                  <span class="panel-info-bar__hint panel-info-bar__hint--placeholder">
                    {{ t('purchaseInvoiceWriteOffWorkspace.selectInvoiceHint') }}
                  </span>
                </template>
              </span>
              <div v-if="selectedInvoice" class="panel-info-bar__actions">
                <span class="panel-info-bar__total" :class="writeOffTotalColorClass">
                  {{
                    t('purchaseInvoiceWriteOffWorkspace.writeOffTotalLabel', {
                ...
                <el-button type="primary" link size="small" @click="autoFillWriteOffAmounts">
                  {{ t('purchaseInvoiceWriteOffWorkspace.autoFill') }}
                </el-button>
              </div>
            </div>
            <div class="receivable-main" :class="{ 'is-overflow': stockInListOverflow }">
              <div ref="stockInScrollRef" class="receivable-scroll">
                <div class="detail-items-table-wrap detail-items-table-wrap--receivable">
                  ...
                  />
                  <DetailListPanelEmpty
                    v-else-if="!stockInTableRows.length"
                    size="medium"
                    :description="t('purchaseInvoiceWriteOffWorkspace.noStockIns')"
                  />
                  <el-table
                    v-else
                    :data="stockInTableRows"
                    class="detail-panel-list-table receivable-panel-table"
                    size="small"
                    stripe
                    :highlight-current-row="embedded"
                    :row-class-name="stockInRowClassName"
                    row-key="rowKey"
                    @row-click="onStockInTableRowClick"
                  >
                    <el-table-column
                      :label="t('purchaseInvoiceWriteOffWorkspace.colStockInCode')"
                      min-width="118"
                      show-overflow-tooltip
                    >
                      <template #default="{ row }">
                        <router-link
                          v-if="row.stockIn.stockInId && row.stockIn.stockInCode"
                          class="link-text"
                          :to="{ name: 'StockInDetail', params: { id: row.stockIn.stockInId } }"
                          @click.stop
                        >
                          {{ row.stockIn.stockInCode }}
                        </router-link>
                        <span v-else>{{ row.stockIn.stockInCode || '—' }}</span>
                      </template>
                    </el-table-column>
                    <el-table-column :label="t('purchaseInvoiceWriteOffWorkspace.colStockInDate')" width="108">
                      <template #default="{ row }">{{ formatDate(row.stockIn.stockInDate) }}</template>
                    </el-table-column>
                    <el-table-column
                      :label="t('purchaseInvoiceWriteOffWorkspace.colItemCode')"
                      min-width="100"
                      show-overflow-tooltip
                    >
                      <template #default="{ row }">{{ row.item.stockInItemCode || '—' }}</template>
                    </el-table-column>
                    <el-table-column
                      :label="t('purchaseInvoiceWriteOffWorkspace.colPurchaseOrderCode')"
                      min-width="118"
                      show-overflow-tooltip
                    >
                      <template #default="{ row }">
                        {{ row.item.purchaseOrderCode || row.stockIn.purchaseOrderCodes || '—' }}
                      </template>
                    </el-table-column>
                    <el-table-column
                      :label="t('purchaseInvoiceWriteOffWorkspace.colPurchaseUser')"
                      width="96"
                      show-overflow-tooltip
                    >
                      <template #default="{ row }">
                        {{ row.item.purchaseUserName || row.stockIn.purchaseUserName || '—' }}
                      </template>
                    </el-table-column>
                    <el-table-column
                      :label="t('purchaseInvoiceWriteOffWorkspace.colFreightForwarder')"
                      min-width="110"
                      show-overflow-tooltip
                    >
                      <template #default="{ row }">
                        {{ row.item.freightForwarderOrderNo || row.stockIn.freightForwarderOrderNo || '—' }}
                      </template>
                    </el-table-column>
                    <el-table-column
                      :label="t('purchaseInvoiceWriteOffWorkspace.colItemToBe')"
                      width="132"
                      min-width="130"
                      align="right"
                    >
                        ...
                        <span class="finance-amount-emphasis finance-amount-emphasis--amount">
                          {{
                            formatAmountWithCurrency(
                              row.item.invoiceMatchToBe,
                              row.item.currency ?? row.stockIn.currency
                            )
                          }}
                        </span>
                      </template>
                    ...
                    <el-table-column
                      fixed="right"
                      :label="t('purchaseInvoiceWriteOffWorkspace.colWriteOffAmount')"
                      width="148"
                      min-width="148"
                      align="center"
                      header-align="center"
                    ...
                    >
                      <template #default="{ row }">
                        <el-input-number
                          v-model="row.item.writeOffAmount"
                          :class="[
                            'write-off-amount-input',
                            { 'write-off-amount-input--positive': isWriteOffAmountPositive(row.item.writeOffAmount) }
                          ]"
                          :min="0"
                          :max="row.item.invoiceMatchToBe"
                          :precision="2"
                          :controls="false"
                          size="small"
                          @click.stop
              ...
              <div v-if="vendorId && canWriteFinancePurchaseInvoice" class="receivable-submit-bar">
                <el-button type="primary" :loading="submitting" @click="submitWriteOff">
                  {{ t('purchaseInvoiceWriteOffWorkspace.submit') }}
                </el-button>
              </div>
            </div>
          </div>
...
import {
  ...
  type FinancePurchaseInvoiceWriteOffInvoiceRow,
...
import {
  readPurchaseInvoiceWriteOffPanelsLayout,
  writePurchaseInvoiceWriteOffPanelsLayout,
  type PurchaseInvoiceWriteOffPanelsLayout
} from '@/utils/purchaseInvoiceWriteOffPanelsLayout'

type StockInItemUiRow = FinancePurchaseInvoiceWriteOffStockInRow['items'][number] & {
  writeOffAmount: number
}

type StockInUiRow = FinancePurchaseInvoiceWriteOffStockInRow & {
  items: StockInItemUiRow[]
}

type StockInTableRow = {
  rowKey: string
  stockIn: StockInUiRow
  item: StockInItemUiRow
}
...
const props = withDefaults(
  defineProps<{
    ...
    embedSummary?: FinancePurchaseInvoiceWriteOffVendorSummary | null
  }>(),
  {
    embedded: false,
    embedVendorId: '',
...
const { purchaseInvoiceTypeLabel } = useFinanceEnumLabels()

const layoutRef = ref<HTMLElement | null>(null)
const mainPairRef = ref<HTMLElement | null>(null)
const stockInScrollRef = ref<HTMLElement | null>(null)
const centerPanelSizePx = ref(0)
const dragging = ref(false)
const panelsLayout = ref<PurchaseInvoiceWriteOffPanelsLayout>(readPurchaseInvoiceWriteOffPanelsLayout())
...
const vendorSummary = ref<FinancePurchaseInvoiceWriteOffVendorSummary | null>(null)
const invoices = ref<FinancePurchaseInvoiceWriteOffInvoiceRow[]>([])
const stockInUiRows = ref<StockInUiRow[]>([])
const selectedInvoiceId = ref('')
const focusedStockInId = ref('')
const stockInListOverflow = ref(false)
let stockInScrollResizeObserver: ResizeObserver | null = null

const vendorNameZh = computed(
  () => vendorSummary.value?.vendorName?.trim() || vendorSummary.value?.vendorId || '—'
)
const vendorNameEn = computed(() => vendorSummary.value?.vendorEnglishName?.trim() || '')

const selectedInvoice = computed(
  () => invoices.value.find((x) => x.id === selectedInvoiceId.value) ?? null
)

const stockInTableRows = computed<StockInTableRow[]>(() => {
  const rows: StockInTableRow[] = []
  for (const stockIn of stockInUiRows.value) {
    for (const item of stockIn.items) {
      rows.push({
        rowKey: item.stockInItemId,
        stockIn,
        item
      })
    }
...
const writeOffAmountTotal = computed(() => {
  ...
  for (const stockIn of stockInUiRows.value) {
    for (const item of stockIn.items) {
      sum += Number(item.writeOffAmount) || 0
    }
  }
  return sum
})
...
const writeOffTotalColorClass = computed(() => {
  ...
  const remaining = selectedInvoice.value?.verifiedToBe ?? 0
...
function setPanelsLayout(layout: PurchaseInvoiceWriteOffPanelsLayout) {
  if (panelsLayout.value === layout) return
  panelsLayout.value = layout
  writePurchaseInvoiceWriteOffPanelsLayout(layout)
  centerPanelSizePx.value = 0
  void nextTick(() => {
    initPanelSizes()
    updateStockInListOverflow()
  })
}
...
function onSplitEnd() {
  ...
  void nextTick(() => updateStockInListOverflow())
}
...
function invoiceTypeLabelForRow(row: FinancePurchaseInvoiceWriteOffInvoiceRow) {
  if (row.redInvoiceStatus === 1) return t('purchaseInvoiceWriteOffWorkspace.redInvoice')
  return purchaseInvoiceTypeLabel(100)
}

function invoiceRowClassName({ row }: { row: FinancePurchaseInvoiceWriteOffInvoiceRow }) {
  return row.id === selectedInvoiceId.value ? 'is-write-off-receipt-selected' : ''
}

function stockInRowClassName({ row }: { row: StockInTableRow }) {
  return row.stockIn.stockInId === focusedStockInId.value ? 'is-piwo-stock-in-focused' : ''
}

function onInvoiceRowSelect(row: FinancePurchaseInvoiceWriteOffInvoiceRow) {
  selectedInvoiceId.value = row.id
  clearWriteOffAmounts()
}

function onStockInTableRowClick(row: StockInTableRow) {
  focusStockIn(row.stockIn)
}

function mapStockIns(rows: FinancePurchaseInvoiceWriteOffStockInRow[]): StockInUiRow[] {
  return (rows ?? []).map((r) => ({
    ...r,
    items: (r.items ?? []).map((item) => ({ ...item, writeOffAmount: 0 }))
  }))
}
...
function focusStockIn(stockIn: StockInUiRow) {
  focusedStockInId.value = stockIn.stockInId
  if (props.embedded) {
    queueStore.setFocusedStockIn(stockIn)
  }
}

function pickDefaultFocusedStockIn() {
  const rows = stockInUiRows.value
  if (!rows.length) {
    focusedStockInId.value = ''
    if (props.embedded) queueStore.setFocusedStockIn(null)
    return
  }
  const still = focusedStockInId.value
    ? rows.find((r) => r.stockInId === focusedStockInId.value)
    : undefined
  const next = still ?? rows[0]
  focusStockIn(next)
}

function updateStockInListOverflow() {
  const scroll = stockInScrollRef.value
  const main = scroll?.parentElement
  if (!scroll || !main) {
    stockInListOverflow.value = false
    return
  }
  ...
  stockInListOverflow.value = scroll.scrollHeight > available + 1
}

function setupStockInScrollObserver() {
  stockInScrollResizeObserver?.disconnect()
  stockInScrollResizeObserver = null
  const scroll = stockInScrollRef.value
  const main = scroll?.parentElement
  if (!scroll || typeof ResizeObserver === 'undefined') return
  stockInScrollResizeObserver = new ResizeObserver(() => {
    void nextTick(() => updateStockInListOverflow())
  })
  stockInScrollResizeObserver.observe(scroll)
  if (main) stockInScrollResizeObserver.observe(main)
  void nextTick(() => updateStockInListOverflow())
}

async function loadCandidates(vid: string, currency: number, resetInvoiceSelection: boolean) {
  vendorId.value = vid
  selectedCurrency.value = currency
  if (props.embedSummary && props.embedSummary.vendorId === vid) {
    vendorSummary.value = props.embedSummary
  }
  loading.value = true
  try {
    const res = await financePurchaseInvoiceWriteOffApi.getCandidates(vid, currency)
    vendorSummary.value = {
      vendorId: res.vendorId,
      vendorName: res.vendorName,
      vendorEnglishName: res.vendorEnglishName,
      currency: res.currency,
      pendingWriteOffTotal: 0,
      pendingInvoiceCount: res.invoices?.length ?? 0,
      hasOpenStockIn: (res.stockIns?.length ?? 0) > 0
    }
    invoices.value = res.invoices ?? []
    stockInUiRows.value = mapStockIns(res.stockIns ?? [])
    ...
    loading.value = false
    void nextTick(() => {
      pickDefaultFocusedStockIn()
      setupStockInScrollObserver()
      updateStockInListOverflow()
    })
  }
}
...
async function syncEmbeddedSelection(resetInvoiceSelection: boolean) {
  const vid = (props.embedVendorId || '').trim()
  if (!vid || props.embedCurrency == null) {
    vendorId.value = ''
  ...
  await loadCandidates(vid, props.embedCurrency, resetInvoiceSelection)
}
...
function autoFillWriteOffAmounts() {
  ...
  if (!invoice) {
    ElMessage.warning(t('purchaseInvoiceWriteOffWorkspace.selectInvoiceFirst'))
    return
  }
  let budget = roundMoney(invoice.verifiedToBe ?? 0)
  if (budget <= 0) {
    ElMessage.warning(t('purchaseInvoiceWriteOffWorkspace.autoFillNoRemaining'))
    return
  }
  clearWriteOffAmounts()
  for (const stockIn of stockInUiRows.value) {
    for (const item of stockIn.items) {
      ...
      const fillAmount = roundMoney(Math.min(item.invoiceMatchToBe ?? 0, budget))
      item.writeOffAmount = fillAmount
...
function buildAllocations() {
  const allocations: { stockInItemId: string; amount: number }[] = []
  for (const stockIn of stockInUiRows.value) {
    for (const item of stockIn.items) {
      const amount = Number(item.writeOffAmount) || 0
      if (amount > 0.0001) {
        allocations.push({ stockInItemId: item.stockInItemId, amount })
      }
    }
  }
  return allocations
...
function validatePayload() {
  ...
  if (!allocations.length) {
    ElMessage.warning(t('purchaseInvoiceWriteOffWorkspace.noAmount'))
    return null
  }
  const total = allocations.reduce((s, a) => s + a.amount, 0)
  if (total > (invoice.verifiedToBe ?? 0) + 0.001) {
    ElMessage.warning(t('purchaseInvoiceWriteOffWorkspace.exceedInvoiceRemaining'))
    return null
  }
  for (const stockIn of stockInUiRows.value) {
    for (const item of stockIn.items) {
      const amount = Number(item.writeOffAmount) || 0
      if (amount > (item.invoiceMatchToBe ?? 0) + 0.001) {
        ElMessage.warning(t('purchaseInvoiceWriteOffWorkspace.exceedStockInItem'))
        return null
      }
    }
  }
  return { financePurchaseInvoiceId: invoice.id, allocations }
}
...
async function submitWriteOff() {
  ...
  try {
    await financePurchaseInvoiceWriteOffApi.apply(payload)
    ElMessage.success(t('purchaseInvoiceWriteOffWorkspace.success'))
    emit('applied')
    const vid = vendorId.value
    const currency = selectedCurrency.value
    if (vid && currency != null) {
      await loadCandidates(vid, currency, false)
    }
  } catch (e: unknown) {
    const msg = e instanceof Error ? e.message : t('purchaseInvoiceWriteOffWorkspace.failed')
    ElMessage.error(msg)
  } finally {
...
watch(
  () =>
    [
      ...
      props.embedCurrency,
      props.embedSummary
        ? `${props.embedSummary.vendorId}::${props.embedSummary.currency ?? 0}`
        : ''
    ] as const,
  () => {
...
watch(
  () => stockInUiRows.value.map((r) => r.stockInId).join('|'),
  () => {
    if (!props.embedded) return
    void nextTick(() => pickDefaultFocusedStockIn())
  }
)

watch(
  () => [stockInTableRows.value.length, vendorId.value, !!selectedInvoice.value] as const,
  () => {
    void nextTick(() => {
      setupStockInScrollObserver()
      updateStockInListOverflow()
...
onBeforeUnmount(() => {
  ...
  stockInScrollResizeObserver = null
  if (props.embedded) {
    queueStore.setFocusedStockIn(null)
  }
...
.write-off-page {
  ...
  .write-off-panel {
    ...
    &--right {
      ...
      :deep(.receivable-panel-table) {
        .el-table__body tr.is-piwo-stock-in-focused > td.el-table__cell {
          background: #fffaea !important;
          background-color: #fffaea !important;
        }

        .el-table__body tr.is-piwo-stock-in-focused:hover > td.el-table__cell {
          background: #fffaea !important;
          background-color: #fffaea !important;
        }
  ...
  .detail-items-table-wrap {
    ...
    :deep(.el-table.detail-panel-list-table) {
      ...
      .el-table__fixed-right .el-table__body tr.is-piwo-stock-in-focused td.el-table__cell,
      .el-table__body tr.is-piwo-stock-in-focused td.el-table-fixed-column--right {
        background: #fffaea !important;
        background-color: #fffaea !important;
      }
    }

<template>
  <div class="sales-order-list-page">
    ...
    <el-card class="table-card">
      ...
      >
        ...
        <el-table-column prop="purchaseOrderStatus" label="采购状态" width="100" align="center">
          <template #default="{ row }">
            <el-tag :type="getPurchaseStatusType(row.purchaseOrderStatus)" size="small">
    ...
    <!-- 查看详情对话框 -->
    <el-dialog v-model="viewDialogVisible" title="销售订单详情" width="820px">
...
const refreshCurrentTags = async () => {
  ...
  try {
    currentTags.value = await tagApi.getEntityTags('SALES_ORDER', currentRow.value.id)
  } catch {
    currentTags.value = []
  }
}
...
const openTagDialog = () => {
  if (!currentRow.value?.id) {
    ElMessage.warning('请先选择要操作的销售订单')
    return
  }

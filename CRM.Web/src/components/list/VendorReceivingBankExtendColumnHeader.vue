<template>
  <div class="vendor-extend-col-header" :class="{ 'is-expanded': expanded }">
    <button
      ...
      :aria-label="
        expanded
          ? t('common.vendorReceivingBankExtendCol.collapse')
          : t('common.vendorReceivingBankExtendCol.expand')
      "
      ...
      >
        ...
        >
          ...
          <span
            ...
            role="separator"
            :aria-label="t('common.vendorReceivingBankExtendCol.resizeSubCol')"
    ...
    <template v-else>
      <div class="vendor-extend-col-header__title">
        <span class="vendor-extend-col-header__label">{{
          t('common.vendorReceivingBankExtendCol.columnTitle')
        }}</span>
        <el-dropdown trigger="click" placement="bottom-start" @command="onFieldCommand">
          <button
            type="button"
            class="vendor-extend-col-field-picker"
            :aria-label="t('common.vendorReceivingBankExtendCol.pickField')"
            @click.stop
          >
            ▾
          </button>
...
import { useVendorReceivingBankExtendColumn } from '@/composables/useVendorReceivingBankExtendColumn'
...
const emit = defineEmits<{
  'set-active-field': [field: VendorReceivingBankExtendFieldKey]
}>()
...
function fieldLabel(key: VendorReceivingBankExtendFieldKey) {
  return t(`common.vendorReceivingBankExtendCol.fields.${key}`)
}

function fieldShort(key: VendorReceivingBankExtendFieldKey) {
  return t(`common.vendorReceivingBankExtendCol.fieldShort.${key}`)
}

function onFieldCommand(cmd: string | number | object) {
  const k = String(cmd) as VendorReceivingBankExtendFieldKey

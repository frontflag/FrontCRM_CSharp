...
/** 数据范围：document/PRD/规范/业务规范/业务员与采购员下拉规范.md §3 */
...
function mapTree(nodes: SalesUserTreeNode[] | undefined): CascaderNode[] {
...
onMounted(async () => {
  try {
    const res = await authApi.getSalesUsersTree()
    // apiClient 拦截器已解包 ApiResponse.data，这里 res 即为树数组；兼容未解包形态
    const raw = res as unknown
    const data = Array.isArray(raw)
      ? (raw as SalesUserTreeNode[])
    ...
    options.value = []
    console.warn('[SalesUserCascader] failed to load /api/v1/auth/sales-users-tree')
  }
})

<template>
  <div
    ...
    aria-label="stock-out-notify-flow-panel"
  >
    <div v-if="!row" class="so-item-flow-root__empty">
      {{ t('stockOutNotifyList.flowPanel.pickRow') }}
    </div>
    ...
    <div v-else v-loading="loading" class="so-item-flow-root__content">
      ...
      <ol class="so-item-flow-timeline">
        <li
          ...
          :class="{ 'is-main': station.key === 'stockOutNotify' }"
        >
          ...
          <div class="so-item-flow-station__body">
            <div class="so-item-flow-station__head">
              <h3 class="so-item-flow-station__title">
                {{ t(station.titleKey) }}
                <FlowYouAreHereMark v-if="station.key === 'stockOutNotify'" />
              </h3>
              <span
...
import {
  ...
  type StockOutNotifyFlowStationKey
} from '@/utils/stockOutNotifyFlowPanel'

const props = withDefaults(
  defineProps<{
    row: Record<string, unknown> | null
    aggregates?: StockOutNotifyFlowAggregates | null
    loading?: boolean
    loadError?: string
...
function isStockItemLike(key: StockOutNotifyFlowStationKey) {
  return key === 'stockItem' || key === 'stockingStockItem'
}

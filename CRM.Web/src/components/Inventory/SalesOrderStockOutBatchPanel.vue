<template>
  <div class="stock-out-batch-panel sales-order-stock-out-batch-panel">
    <div class="batch-panel-head">
      <h3 class="batch-panel-title">{{ t('salesOrderDetailView.batchPanel.title') }}</h3>
      <span v-if="activeInnerTab === 'list'" class="batch-panel-count">
        {{ t('salesOrderDetailView.batchPanel.count', { count: listTotalServer }) }}
      </span>
    </div>
    ...
    <div class="batch-panel-tabs">
      <div class="batch-panel-tabs-bar">
        <div class="batch-panel-tabs-nav">
          ...
          >
            {{ t('salesOrderDetailView.batchPanel.tabs.list') }}
          </button>
          <button
            type="button"
            class="batch-tab-btn"
          ...
          >
            {{ t('salesOrderDetailView.batchPanel.tabs.exportLogs') }}
          </button>
        </div>
      ...
      <div v-show="activeInnerTab === 'list'" class="batch-panel-body">
        <div class="detail-items-table-wrap">
          <CrmDataTable
            ref="listTableRef"
            column-layout-key="sales-order-detail-batch-panel-v1"
      ...
      <div v-show="activeInnerTab === 'logs'" class="batch-panel-body">
        <div class="detail-items-table-wrap">
          <CrmDataTable
            column-layout-key="sales-order-detail-batch-export-logs-v1"
...
import salesOrderApi, { type SalesOrderBatchExportLogRow } from '@/api/salesOrder'
...
const logRows = ref<SalesOrderBatchExportLogRow[]>([])
...
const logTableColumns = computed<CrmTableColumnDef[]>(() => [
  {
    key: 'operationTime',
    label: t('salesOrderDetailView.batchPanel.exportLogs.columns.operationTime'),
    prop: 'operationTime',
    width: 130
  },
  {
    key: 'operatorUserName',
    label: t('salesOrderDetailView.batchPanel.exportLogs.columns.operator'),
  ...
  {
    key: 'exportedCount',
    label: t('salesOrderDetailView.batchPanel.exportLogs.columns.exportedCount'),
    prop: 'exportedCount',
    width: 100,
    align: 'right'
  },
  ...
  {
    key: 'operationDesc',
    label: t('salesOrderDetailView.batchPanel.exportLogs.columns.operationDesc'),
    prop: 'operationDesc',
    minWidth: 220,
    showOverflowTooltip: true
  }
...
async function fetchList(resetPage = true) {
  const id = (props.salesOrderId ?? '').trim()
  if (!id) {
    list.value = []
  ...
  try {
    ...
    ElMessage.error(getApiErrorMessage(e, t('salesOrderDetailView.batchPanel.messages.loadListFailed')))
    list.value = []
    listTotalServer.value = 0
  } finally {
...
async function fetchLogs(resetPage = true) {
  ...
  try {
    const paged = await salesOrderApi.getBatchExportLogs(id, {
      page: logsPage.value,
    ...
    ElMessage.error(getApiErrorMessage(e, t('salesOrderDetailView.batchPanel.messages.loadExportLogsFailed')))
    logRows.value = []
    logsTotal.value = 0
  } finally {
...
async function exportBatches() {
  const id = (props.salesOrderId ?? '').trim()
  const code = (props.salesOrderCode ?? '').trim()
  if (!id) return
  try {
    await ElMessageBox.confirm(
      t('salesOrderDetailView.batchPanel.messages.exportConfirmMessage', {
        code: code || '—',
        count: listTotalServer.value
      }),
      t('salesOrderDetailView.batchPanel.messages.exportConfirmTitle'),
      {
        type: 'warning',
  ...
  try {
    const blob = await batchReconciliationApi.exportOutBatches({
      sellOrderId: id,
      exportSource: 'salesOrder',
      exportPageUrl: `/sales-orders/${id}`
    })
    ...
    ElMessage.error(getApiErrorMessage(e, t('salesOrderDetailView.batchPanel.messages.exportFailed')))
  } finally {

<template>
  <div
    ...
    aria-label="stock-out-item-flow-panel"
  >
    <div v-if="!row" class="so-item-flow-root__empty">
      {{ t('stockOutItemList.flowPanel.pickRow') }}
    </div>
    ...
    <div v-else v-loading="loading" class="so-item-flow-root__content">
      ...
      <ol class="so-item-flow-timeline">
        <li
          ...
          :class="{ 'is-main': station.key === 'stockOut' }"
        >
          ...
          <div class="so-item-flow-station__body">
            <div class="so-item-flow-station__head">
              <h3 class="so-item-flow-station__title">
                {{ t(station.titleKey) }}
                <FlowYouAreHereMark v-if="station.key === 'stockOut'" />
              </h3>
              <span
              ...
              <article v-for="card in station.cards" :key="card.id" class="so-item-flow-card">
                <div class="so-item-flow-kv">
                  ...
                  <div v-if="station.key === 'stockOut'" class="so-item-flow-kv__cell">
                    <span class="so-item-flow-kv__label">
                      {{ t('inventoryStockItemList.flowPanel.fields.stockOutType') }}：
                    </span>
                  ...
                  <div
                    ...
                    :class="{ 'so-item-flow-kv__cell--full': isStockItemLike(station.key) }"
                  >
                  ...
                  <div
                    v-if="isStockItemLike(station.key)"
                  ...
                  <div
                    v-else-if="station.key !== 'stockOut' && card.bizTypeLabelKey"
                    class="so-item-flow-kv__cell"
                  >
                  ...
                  <div
                    v-if="station.key === 'stockOut' || (!card.bizTypeLabelKey && card.showPerson)"
                    class="so-item-flow-kv__cell"
                  >
...
import type { StockOutItemFlowAggregates } from '@/api/stockOut'
...
import type { FlowDocRoute, FlowStationStatus } from '@/utils/stockItemFlowPanel'
import {
  buildStockOutItemFlowStations,
  formatStockOutItemFlowCardDate,
  type StockOutItemFlowStationKey
} from '@/utils/stockOutItemFlowPanel'

const props = withDefaults(
  defineProps<{
    row: Record<string, unknown> | null
    aggregates?: StockOutItemFlowAggregates | null
    loading?: boolean
    loadError?: string
...
const stations = computed(() =>
  buildStockOutItemFlowStations(props.row, props.aggregates, t as (key: string, ...args: unknown[]) => string, {
    maskPurchase: props.maskPurchase,
    maskSale: props.maskSale
  })
)

function isStockItemLike(key: StockOutItemFlowStationKey) {

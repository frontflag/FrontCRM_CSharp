...
import { authApi, type BusinessOpsStaffUserOption } from '@/api/auth'
...
const options = ref<BusinessOpsStaffUserOption[]>([])
...
onMounted(async () => {
  loading.value = true
  try {
    options.value = await authApi.getBusinessOpsStaffUsers()
  } catch {
    options.value = []
  } finally {
    loading.value = false

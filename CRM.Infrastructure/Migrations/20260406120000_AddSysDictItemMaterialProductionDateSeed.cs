...
{
    /// <summary>物料「生产日期」字典（sys_dict_item.MaterialProductionDate）</summary>
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20260406120000_AddSysDictItemMaterialProductionDateSeed")]
    public partial class AddSysDictItemMaterialProductionDateSeed : Migration
    {
        protected override void Up(MigrationBuilder migrationBuilder)
        {
            foreach (var row in SysDictMaterialSeedRows.All)
            {
...
WHERE NOT EXISTS (SELECT 1 FROM public.sys_dict_item d WHERE d.""Category"" = '{cat}' AND d.""ItemCode"" = '{code}');");
        ...
        {
            migrationBuilder.Sql(
                @"DELETE FROM public.sys_dict_item WHERE ""Category"" = 'MaterialProductionDate';");
        }

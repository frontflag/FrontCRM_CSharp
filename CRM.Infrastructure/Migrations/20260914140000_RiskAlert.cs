...
[Migration("20260914140000_RiskAlert")]
public partial class AddRiskAlert : Migration
{
...
");
    ...
    {
        migrationBuilder.Sql(@"
DROP TABLE IF EXISTS public.risk_alert_setting;
DELETE FROM sys_role_permission WHERE ""PermissionId"" IN (
  SELECT ""PermissionId"" FROM sys_permission WHERE ""PermissionCode"" IN ('system.params.risk-alert.read', 'system.params.risk-alert.write')
);
DELETE FROM sys_permission WHERE ""PermissionCode"" IN ('system.params.risk-alert.read', 'system.params.risk-alert.write');
");
    }
}

...
[Migration("20260914160000_IndustryNewsBriefing")]
public partial class AddIndustryNewsBriefing : Migration
{
...
""");
    ...
    {
        migrationBuilder.Sql(@"
DROP TABLE IF EXISTS public.industry_news_briefing;
DELETE FROM sys_role_permission WHERE ""PermissionId"" IN (
  SELECT ""PermissionId"" FROM sys_permission WHERE ""PermissionCode"" = 'biz.ai.industry_news.briefing'
);
DELETE FROM sys_permission WHERE ""PermissionCode"" = 'biz.ai.industry_news.briefing';
DELETE FROM public.ai_scenario WHERE code = 'industry.news.briefing';
DELETE FROM public.ai_prompt_template WHERE code = 'industry.news.briefing';
");
    }
}

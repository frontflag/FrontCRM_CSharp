...
{
    /// <summary>
    /// 入库批次 V2：全局编号 PC-xxxxxxxx、批次维度/单位/数量；移除 LOT/SN 分列数量与 stock_in_id 冗余。
    /// </summary>
    [DbContext(typeof(ApplicationDbContext))]
    [Migration("20260605130000_StockInBatchV2Redesign")]
    public partial class StockInBatchV2Redesign : Migration
    {
...
COMMENT ON TABLE public.stock_in_batch IS '入库批次：关联入库明细，全局编号 PC-xxxxxxxx';
COMMENT ON COLUMN public.stock_in_batch.global_batch_no IS '批次全局唯一编号 PC- + 8位十进制';
...
SELECT gen_random_uuid()::text, 'InventoryBatchDimension', v.code, v.zh, v.en, v.ord, true, NOW() AT TIME ZONE 'utc'
FROM (VALUES
    ('LOT', 'LOT', 'LOT', 10),
    ('SN', 'SN', 'SN', 20),
    ('REEL', '卷盘', 'REEL', 30),
...
WHERE NOT EXISTS (
    ...
    WHERE d.""Category"" = 'InventoryBatchDimension' AND d.""ItemCode"" = v.code);

INSERT INTO public.sys_dict_item (""Id"",""Category"",""ItemCode"",""NameZh"",""NameEn"",""SortOrder"",""IsActive"",""CreateTime"")
SELECT gen_random_uuid()::text, 'InventoryBatchRecordUnit', v.code, v.zh, v.en, v.ord, true, NOW() AT TIME ZONE 'utc'
FROM (VALUES
    ('PCS', 'PCS', 'PCS', 10),
    ('EA', 'EA', 'EA', 20),
    ('TUBE', '管', 'TUBE', 30),
...
WHERE NOT EXISTS (
    SELECT 1 FROM public.sys_dict_item d
    WHERE d.""Category"" = 'InventoryBatchRecordUnit' AND d.""ItemCode"" = v.code);
");
        }
        ...
        {
            migrationBuilder.Sql(@"
DELETE FROM public.sys_dict_item WHERE ""Category"" IN ('InventoryBatchDimension', 'InventoryBatchRecordUnit');
DELETE FROM sys_serial_number WHERE ""ModuleCode"" = 'InventoryBatch';
DROP TABLE IF EXISTS public.stock_in_batch;
");
        }
    }

...
[Migration("20260913180000_WorkCalendarAndTask")]
public partial class WorkCalendarAndTask : Migration
{
...
");
    ...
    {
        migrationBuilder.Sql(@"
DROP TABLE IF EXISTS public.work_task;
DROP INDEX IF EXISTS public.ix_rfq_sales_assigned;
DROP INDEX IF EXISTS public.ix_sellorder_sales_approved;
ALTER TABLE public.rfq DROP COLUMN IF EXISTS assigned_at;
ALTER TABLE public.sellorder DROP COLUMN IF EXISTS approved_at;
DELETE FROM sys_role_permission WHERE ""PermissionId"" IN (
  SELECT ""PermissionId"" FROM sys_permission WHERE ""PermissionCode"" IN ('work-task.read', 'work-task.write')
);
DELETE FROM sys_permission WHERE ""PermissionCode"" IN ('work-task.read', 'work-task.write');
");
    }
}

...
public static class CustomerNewsMarkdownSanitizer
{
    ...
    public const string OutputHardConstraint =
        "【输出硬约束】第一行必须是「## 核心摘要」。禁止输出检索过程，禁止写「让我继续搜索」「让我调整搜索策略」「让我进行一次更全面的搜索」及同义反复。"
        + "必须按公司全称检索公开信息（含是否上市及交易所/巨潮公告），不得把输入里的「未知」当成未上市。"
        + "仅当某维度检索后窗口内确实没有信息时，该维度才写「本期无重大事项」。禁止因一次检索无结果就停止。禁止用窗口外旧闻充数。"
        + "写完「待核实区」后必须停止，禁止重复同一句话。";

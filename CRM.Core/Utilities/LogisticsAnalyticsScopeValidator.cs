...
/// <summary>物流看板 viewLevel / departmentId / ownerUserId 校验；支持物流岗（A 类）与仅销/采 Scope（B 类）。</summary>
public static class LogisticsAnalyticsScopeValidator
{
    public const string UnassignedCustomerAnchorId = "__unassigned_customer__";
    public const string UnassignedCustomerDisplayName = "无客户 / 备货";

    public static bool IsSalesPurchaseOnlyMode(UserPermissionSummaryDto summary)
    {
        if (summary.IsSysAdmin) return false;
        if (summary.LogisticsDataScope is 0 or 1 or 2 or 3) return false;
    ...
    public static bool CanAccessPage(UserPermissionSummaryDto summary)
    {
        ...
        if (summary.LogisticsDataScope is 0 or 1 or 2 or 3) return true;
    ...
    public static IReadOnlyList<string> GetAllowedViewLevels(UserPermissionSummaryDto summary)
    {
        ...
        return summary.LogisticsDataScope switch
        {
            0 => AllThree(),
            3 => AllThree(),
    ...
    public static string GetDefaultViewLevel(UserPermissionSummaryDto summary)
    {
        ...
        if (summary.IsSysAdmin || summary.LogisticsDataScope == 0) return SalesAnalyticsViewLevels.Company;
        if (summary.LogisticsDataScope == 3) return SalesAnalyticsViewLevels.Department;
        if (summary.LogisticsDataScope == 2) return SalesAnalyticsViewLevels.Department;
        return SalesAnalyticsViewLevels.Personal;
    }
        ...
        HashSet<string> salesPurchaseLensUserIds)
    {
        ...
        if (allowedLevels.Count == 0)
            return new ValidationResult { Ok = false, Error = "当前账号无库存分析数据范围" };
        ...
        if (level == SalesAnalyticsViewLevels.Personal)
        {
            ...
            else if (!string.IsNullOrWhiteSpace(ownerUserId))
            {
                ...
                if (salesPurchaseLensUserIds.Count > 0 && !salesPurchaseLensUserIds.Contains(uid))
                    return new ValidationResult { Ok = false, Error = "无权查看该用户库存数据" };
                resolvedUser = uid;
            }
        }
        else if (level == SalesAnalyticsViewLevels.Department)
        {
            if (summary.LogisticsDataScope == 2)
                resolvedDept = summary.PrimaryDepartmentId;
            else if (!string.IsNullOrWhiteSpace(departmentId))
            {
                var did = departmentId.Trim();
                if (!IsDepartmentAllowedForLogistics(summary, did, departments))
                    return new ValidationResult { Ok = false, Error = "无权查看该部门数据" };
                resolvedDept = did;
            }
            else if (summary.LogisticsDataScope is 0 or 3)
                resolvedDept = summary.PrimaryDepartmentId;
            else
                resolvedDept = summary.PrimaryDepartmentId;
        }
        ...
        IReadOnlyList<RbacDepartment> departments)
    {
        if (IsSalesPurchaseOnlyMode(summary))
        {
            var mapped = MapLogisticsScopeToSalesScope(summary);
        ...
        return SalesAnalyticsScopeValidator.BuildAllowedDepartments(
            MapLogisticsScopeToSalesScope(summary), departments);
    }
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        if (summary.LogisticsDataScope is 2 or 3)
        {
            return await dataPermission.GetAllowedUserIdsForDataScopeAsync(
                MapLogisticsScopeToSalesScope(summary),
                includeChildren: summary.LogisticsDataScope == 3,
                cancellationToken);
        }
    ...
    private static UserPermissionSummaryDto MapLogisticsScopeToSalesScope(UserPermissionSummaryDto summary) =>
        new()
        {
            UserId = summary.UserId,
            IsSysAdmin = summary.IsSysAdmin,
            SaleDataScope = summary.LogisticsDataScope,
    ...
    private static bool IsDepartmentAllowedForLogistics(
        UserPermissionSummaryDto summary,
        string departmentId,
        IReadOnlyList<RbacDepartment> departments) =>
        SalesAnalyticsScopeValidatorIsDeptAllowed(MapLogisticsScopeToSalesScope(summary), departmentId, departments);

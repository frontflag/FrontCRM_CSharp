...
new_block = r'''        private List<StockInListItemDto> ApplyStockInMemoryFilters(
...
path = Path(r'd:\MyProject\FrontCRM_CSharp\CRM.Core\Services\StockInService.cs')
text = path.read_text(encoding='utf-8')
start = text.index('        public async Task<IReadOnlyList<StockInListItemDto>> GetListAsync(StockInQueryRequest? request = null)')
end = text.index('        public async Task<StockIn> UpdateAsync(string id, UpdateStockInRequest request, string? actingUserId = null)')

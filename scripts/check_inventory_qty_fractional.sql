...
UNION ALL SELECT 'stockin', COUNT(*) FROM public.stockin
WHERE ("TotalQuantity" - trunc("TotalQuantity")) <> 0

UNION ALL SELECT 'stockinitem', COUNT(*) FROM public.stockinitem
WHERE ("Quantity" - trunc("Quantity")) <> 0
   OR ("OrderQty" - trunc("OrderQty")) <> 0
   OR ("QtyReceived" - trunc("QtyReceived")) <> 0

UNION ALL SELECT 'stockout', COUNT(*) FROM public.stockout
WHERE ("TotalQuantity" - trunc("TotalQuantity")) <> 0

UNION ALL SELECT 'stockoutitem', COUNT(*) FROM public.stockoutitem
WHERE ("Quantity" - trunc("Quantity")) <> 0
   OR ("OrderQty" - trunc("OrderQty")) <> 0

-- 增量：vendor.intel.lookup 场景 + vendor_intel_report 表（已有 AI 模块库执行）
-- DBeaver-safe：Prompt 占位符用 CHR(123)/CHR(125) 拼接，脚本内勿写双花括号字面量
-- 13 章契约与客户情报一致；Prompt 视角为采购与供应链
...
-- 注意：勿使用 ...000b（prompt/scenario 已被名片解析占用）、...0cb（权限已被名片创建客户占用）
...
VALUES (
    ...
    '请调查以下供应商的公开信息：企业名称 '
        ...
        || '。返回 13 章供应商情报 JSON（registry、ownership、business、scale、certifications、timeline、contacts、compliance_risks、market_risks、procurement_signals、opportunities、key_people、ai_assessment）。',
    'json',

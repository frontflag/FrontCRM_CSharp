...
--       purchaserequisition 创建时间为 "CreateTime"（PascalCase），非 create_time。
...
-- ORDER BY pr."CreateTime", pr."PurchaseRequisitionId", poi.purchase_order_item_code;

-- ========== 示例：手动绑定 PO0005Z-1 → 实际生成它的 PR（允许覆盖错误绑定）==========
...
-- 若 Updated Rows = 0：purchase_requisition_id 可能已等于目标 PR，或单号不匹配；见 scripts/fix_pr_status_so00044_postgresql.sql 诊断段

-- ========== FIFO 自动回填（仅 purchase_requisition_id IS NULL 的 PO 行）==========
-- 注意：本段与下方「修正 PR.status」须分开执行；仅跑手动 UPDATE 不会改 status。
WITH pr_ordered AS (
    SELECT

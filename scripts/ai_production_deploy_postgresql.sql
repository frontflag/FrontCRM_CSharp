-- =============================================================================
-- AI 模块 — 生产环境一次性部署脚本（PostgreSQL）
-- 适用：FrontCRM 生产库首次启用 AI，或从旧版 AI 脚本升级
-- 可重复执行（CREATE IF NOT EXISTS / ON CONFLICT / ADD COLUMN IF NOT EXISTS）
--
-- 执行后还需（非 SQL）：
--   1. API 环境变量 AI_MOONSHOT_API_KEY
--   2. systemd EnvironmentFile 或 crm-api.env
--   3. Nginx /api/v1/ai/invoke proxy_read_timeout >= 300s
--   4. 部署含 AI 代码的最新 API + Web
--   5. 用户重新登录（权限写入后）
-- =============================================================================
...
VALUES
    ...
    ('a1000001-0000-4000-8000-000000000002', 'moonshot', 'Kimi (Moonshot)', 'https://api.moonshot.cn/v1', 'AI_MOONSHOT_API_KEY', 'kimi-k2.5', 180, true)
ON CONFLICT (code) DO NOTHING;

UPDATE public.ai_provider
SET base_url = 'https://api.moonshot.cn/v1',
    api_key_env = 'AI_MOONSHOT_API_KEY',
...
-- material.spec.lookup（Debug 物料规格）
...
-- material.intel.lookup（RFQ 物料情报，中文输出）
INSERT INTO public.ai_prompt_template (id, code, version, system_prompt, user_prompt_template, output_format, json_schema_hint, is_active)
VALUES (
...
SET system_prompt = '你是面向中国采购与销售用户的电子元器件情报助手。根据型号仅返回合法 JSON（禁止 markdown 代码块）。JSON 键名必须保持英文 snake_case 不变。所有描述性字符串值（category、part_number_breakdown[].meaning、technical_features[]、application_areas[]、alternatives、pricing、industry_news、disclaimer 等）必须使用简体中文，禁止用英文句子描述（品牌/原厂/官方型号代号可保留英文）。即使联网检索到英文网页，也必须先翻译为简体中文再写入 JSON。未知用 null，无数据用空数组。不要编造价格、库存、URL 或新闻。disclaimer 用中文说明仅供参考，请以原厂或授权渠道规格为准。',
    user_prompt_template = convert_from(
        decode(
            '',
            'hex'
        ),
        'UTF8'
    ),
...
-- 生产推荐：RFQ 物料情报走 Kimi 联网
UPDATE public.ai_scenario
SET provider_code = 'moonshot',
    model = 'kimi-k2.6',
    max_tokens = 16384,
    temperature = 1.00,
...
-- SYS_ADMIN / biz_all：AI 管理 + Debug 规格查询
...
-- RFQ 物料情报：管理员 + 已有 rfq.read 的角色（与代码 RbacService 一致）
INSERT INTO sys_role_permission ("RolePermissionId", "RoleId", "PermissionId", "CreateTime")
SELECT gen_random_uuid()::text, r."RoleId", p."PermissionId", NOW()
FROM sys_role r
...
-- ========== 7. 升级后清缓存（提示词/联网变更后建议执行）==========

DELETE FROM public.ai_invocation_cache WHERE scenario_code IN ('material.intel.lookup', 'material.spec.lookup');

-- ========== 8. 验证（可选，执行后查看结果）==========
-- SELECT code, base_url, default_model FROM ai_provider;
-- SELECT code, provider_code, model, max_tokens, enable_web_search, is_enabled FROM ai_scenario;
-- SELECT code, version, left(system_prompt, 40) FROM ai_prompt_template WHERE code LIKE 'material.%';
-- SELECT "PermissionCode" FROM sys_permission WHERE "PermissionCode" LIKE 'biz.ai.%';

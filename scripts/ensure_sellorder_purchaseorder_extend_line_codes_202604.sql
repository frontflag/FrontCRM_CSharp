-- Idempotent-ish: 与 Migration 20260409120000 对齐，用于手工对齐库结构。
-- PostgreSQL
...
WHERE u."SellOrderItemId" = n."SellOrderItemId"
  AND (u.sell_order_item_code IS NULL OR u.sell_order_item_code = '');
...
WHERE u."PurchaseOrderItemId" = n."PurchaseOrderItemId"
  AND (u.purchase_order_item_code IS NULL OR u.purchase_order_item_code = '');
...
-- 仅当无 NULL 时执行 NOT NULL（若仍有 NULL 请先排查数据）

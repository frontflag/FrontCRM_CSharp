...
饼图可按 **数量（PCS）**、**金额** 或 **库存条目**（当前筛选下的明细条数，每条计 1）查看：库存类型、仓库、业务员、库龄（与物流分析相同分桶：0–30 / 31–90 / 91–180 / 181–365 / 365 天以上）。金额下拉里：

- **USD** 只统计采购原币就是美元的库存。
- **折算USD** 把各原币都按入库当时记下的折算美金单价换成美元再加总，不是打开页面当天的汇率。

没有入库日的行不进库龄图。若筛选包含已出清行，库存条目也会计入这些行。

---

<h2 id="help-isi-board-rankings">排行</h2>

Top10：客户、业务员、物料（型号+品牌）、品牌。可在 **数量（PCS）**、**金额**（各原币或折算USD）、**库存条目**（明细条数）间切换。

**点击任一行** 会回到本页列表，只看该排行对应的库存明细，并带上当前搜索条件。选折算USD 时不过滤原币。
